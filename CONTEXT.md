...
- `ED2K 下载批次`：管理员一次提交的一组 `ed2k://` 链接形成一个批次。批次是管理、回看和汇总状态的一级对象；批次内部再拆成多条独立下载任务，允许同批次内出现部分成功、部分失败和部分仍在下载的混合状态。
...
- `ED2K 批次目录隔离`：[[ED2K 暂存工作区]] 首期按 [[ED2K 下载批次]] 建立顶层目录，例如一个批次对应一个独立子目录；批次内各下载任务再使用各自的子目录或文件名空间。不同批次的下载结果不应直接平铺到同一个公共目录，避免同名文件、残留文件和后续清理/导入归属混乱。
