...
- `TV 短视频单条失败留在当前页`：如果当前短视频播放失败，页面不自动退回首页，也不自动跳过到下一条，而是在当前沉浸式播放页上显示极简失败态。失败态只保留 `重试` 主动作和 `BACK` 退出路径，不扩展为多操作面板。
...
- `TV 短视频保留中央播放暂停提示`：虽然短视频页整体保持沉浸式纯播放，但当用户按 `OK` 切换播放/暂停时，仍保留现有中央短提示，用于明确区分“当前是已暂停”还是“播放卡住”。该提示只服务播放/暂停切换，不扩展到切条成功、自动连播成功等其它动作。
