...
- `TV 长视频 TextureView 输出层`：TV 长视频 ExoPlayer 内核的 `PlayerView` 必须固定 `surface_type="texture_view"`，并保持 reset 时不主动用 shutter 清掉最后一帧。默认 `SurfaceView` 在 Compose 导航返回或 `AndroidView` 销毁时可能先脱离合成层并露出播放器页黑底，表现成退出播放回详情页“闪黑”；这不是 LibVLC/ExoPlayer 选择问题，而是视频输出层合成策略问题。IPTV 直播仍按 [[IPTV LibVLC 路径]] 独立使用 LibVLC 输出策略。
