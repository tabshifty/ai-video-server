...
- `DV 专用链路核心互动层`：[[杜比视界专用系统播放链路]] 的播放会话应保留 TV 长视频的核心遥控体验，包括播放/暂停、快进快退、进度反馈、返回二次确认、续播提示、剧集选集轨与连播提示；字幕/音轨切换不属于本阶段核心互动层。
- `DV 字幕音轨后续阶段`：DV 专用 Media3/ExoPlayer 链路的字幕与音轨选择属于下一阶段能力，不作为 [[DV 专用链路核心互动层]] 的验收范围；后续接入前需单独确认轨道枚举、偏好持久化与 ASS/libass 语义差异。
- `DV 字幕音轨入口隐藏`：在 [[DV 字幕音轨后续阶段]] 完成前，DV 专用播放控制层不显示字幕和音轨入口；不要用禁用按钮或“暂不支持”提示占位。普通 LibVLC 分支继续保留既有字幕和音轨入口。
...
- `admin 短视频清理审核`：管理员在管理端连续查看 `ready` 状态短视频并判断是否保留的清理流程，待审集合按上传时间倒序排列；它只覆盖短视频，不是全量视频审核、上传发布审核、刮削确认或内容分级审核。判定为不需要的视频进入视频删除语义，不新增“通过/驳回”状态；删除当前短视频后，审核流应自动进入下一条待审短视频。重新进入审核页时应优先恢复到上次正在审核的短视频；若该视频已不存在或不再属于待审集合，则回到当前待审集合的第一条。`保留并下一条` 只推进当前审核位置，不写入“已审核”状态，也不让该视频永久退出待审集合；`删除并下一条` 必须先二次确认，再进入不可恢复的视频删除语义。走到当前已加载列表末尾时应自动加载下一页；只有确认没有更多待审短视频时，才展示已到末尾的空状态，并提供回到第一条和刷新列表入口。播放器进入页面和切换短视频后自动播放，默认静音，并允许管理员在当前审核会话内切换声音状态。审核页在桌面和平板优先作为单屏工作台使用，浏览器窗口不应因为播放器或队列高度出现纵向滚动；待审队列在自身面板内部滚动，右侧播放器随页面可用高度收缩。首版只提供关键词搜索，不提供类型或状态筛选，因为待审集合已经固定为 `short + ready`。
