...
- `媒体存储空间优先`：本项目在视频处理和播放兼容策略上以预留更多存储空间为第一要义；不能为了 Dolby Vision 原生播放、兼容回退或调试便利长期保留额外大体积视频副本。任何会增加持久视频副本的方案都必须默认排除，唯一例外是 [[杜比视界剧集原始文件保留]]——对转码时类型为 episode 且源文件被探测为 Dolby Vision 的视频，保留原始文件作为专用链路播放源。
...
- `杜比视界剧集原始文件保留`：转码时视频类型为 episode 且源文件被探测为 Dolby Vision 的视频，跳过 uploadGC 删除原始文件，改为将原始文件移动到 `storageRoot/videos/<uuid>/source-dv.<ext>`（保留原始扩展名），作为 [[杜比视界专用系统播放链路]] 的播放源。该保留仅对剧集生效——movie、short 等其它类型仍按 [[杜比视界原始文件不保留约束]] 在转码完成后删除原始文件。保留条件以转码时 `video.Type` 和源文件 Dolby Vision 探测结果为准，不按文件名、文件大小或后续元数据修改回溯。
- `杜比视界剧集播放源路径`：DV 剧集的原始文件保留后，其在 storageRoot 内的完整路径写入 `playback_compat.source_playback_path` 字段。TV 端通过该字段判断是否有可用的 DV 原始播放源，并通过 `profile=dv_source` 请求该文件；该字段缺失时 TV 端不假设存在 DV 播放源。
