...
- 验证：`bash -n scripts/ed2k-amule-executor.sh` 通过；`git diff --check -- scripts/ed2k-amule-executor.sh CONTEXT.md docs/家用部署机.md plan.md` 通过；`go test ./internal/queue ./internal/handlers ./internal/repository ./internal/config -run 'Ed2k|ed2k|LifecycleMigration' -count=1` 通过；两组本地 fake `amulecmd/ed2k` 脚本模拟通过（覆盖 submit fallback 与 status 自愈补提）；待执行部署机回归。

## 2026-07-02 10:18 +0800
- 进度：排查部署机 ED2K 下载工作台“等待 aMule 同步下载状态”卡死。已确认前端/后端/worker/部署同步都正常，问题不在页面轮询；部署机 `amulecmd status` 显示已连上 ED2K/Kad，但原始任务长时间 `running + 等待 aMule 同步下载状态`。远程复现发现 `scripts/ed2k-amule-executor.sh` 的 `submit` 仅依赖 `ed2k` helper 退出码；在当前部署机上 helper 会输出 `Link successfully queued.`，但 aMule 日志里原始提交时没有“正在添加链接”记录、`show dl` 也为空，导致 worker 只会永久轮询 `show dl`。手工 `amulecmd add <ed2k://...>` 后，`show dl` 立即出现对应 hash/file，说明可恢复路径是“补提到下载队列”，不是前端假象。
- 影响文件：预计涉及 `scripts/ed2k-amule-executor.sh`、`docs/家用部署机.md`、`CONTEXT.md`、`plan.md`
- 验证：已完成部署机只读取证（`/healthz`、`server.log`、`worker.log`、`~/.aMule/logfile`、远端 `amulecmd status/show dl/show log`、远端 DB 查询）；待执行本地脚本语法校验、ED2K Go 定向测试、部署机实机回归（现有 stuck 任务自动转入 `show dl` 可见态）。
...
## 2026-07-02 +0800
- 进度：TV 海报墙（`TvPosterWallScreen`，用户口语称"书架墙"）卡片重设计——master 基线重做。用户反馈卡片丑且太大。此前在一个基于旧基线（`41d53ee`）的 worktree 做过一轮，但 master 随后落下 47 个提交，其中 `a73b6b9 TV 焦点反馈精简为只缩放，剥离光晕与提亮` 删除了 `tvFocusableGlow` 函数、把 `tvFocusableScaleOnly` 签名收敛为 `(enabled, focusedScale)` 无 `shape`、剥离所有暖金光晕/提亮；旧 worktree 设计前提（"保留暖金光晕"）与 master 新口径冲突，无法合并，已丢弃并在 master 最新代码（`b483605`）上重做。主仓库 `CONTEXT.md` 第 721 行已有一处未提交改动（海报墙契约改述为渐变遮罩 + 中性落影例外），用户确认"实现这个契约"——本次代码即让该契约落地。5 项定稿：①焦点放大收口海报墙本地 `posterWallFocusedScale = 1.08f`（不改共享 `TvFocusSafeSpec.posterFocusedScale` 恒 1.04f，避免连带改首页/目录页）+ 中性（黑色）落影抬升 `posterWallFocusedShadowElevationDp` 作为 [[TV 焦点反馈只缩放]] 全仓口径的允许例外（内联在卡片，不抽共享修饰器、不动 `tvFocusableScaleOnly` 签名）；②标题 D-轻渐变遮罩（`TvPosterWallTitleScrimBrush` 三段：透明→Canvas 0.55→0.88，盖底部 45%），删 74dp 独立深色标题色块 `TvPosterWallTitleBackground`，卡片整体高度从"9:16+74dp"缩回纯 9:16；③网格 `GridCells.Adaptive(minSize)` 170dp→90dp，目标 6 列（4K TV 按 1080p 渲染、density~3.0）；④`gridTopPaddingDp` 8f→16f 覆盖纯 9:16 + 1.08 放大顶行聚焦的最坏垂直溢出（`requiredSafeSpaceDp(146×16/9, 1.08, 2)=12.38dp`）；⑤shared-element `tvSharedSeriesPoster` 下沉只绑海报图片节点（与详情页 `TvSeriesDetailBackdrop` 纯背景图节点对应），scrim/标题作兄弟覆盖层、不进 shared-element 子树，修复"标题飞进详情页"回归；占位卡（无海报）只显示「暂无海报」、不叠底部遮罩+真实标题，修复双标签堆叠。
- 实施要点：卡片从 `Column { 海报Box; 标题Box(74dp+深色底) }` 重写为单个 9:16 `Box { AsyncImage/占位(fillMaxSize+sharedElement); scrimBox(fillMaxHeight(0.45f)+align(BottomCenter)); 标题Text(align(BottomStart)+Shadow) }`；中性落影用 `onFocusChanged` 读焦点 + `animateDpAsState` 驱动 elevation + `.shadow(ambientColor/spotColor=Color.Black)`，modifier 顺序 `padding → tvFocusableScaleOnly → onFocusChanged → shadow → clickable`；`tvFocusableScaleOnly` 调用不传 `shape`（master 已删该参数）；删 import `heightIn`，新增 `fillMaxHeight`/`geometry.Offset`/`graphics.Shadow`/`draw.shadow`/`animation.core.animateDpAsState`/`animation.core.spring`/`focus.onFocusChanged`/`core.ui.TvFocusMotionTokens`。
- 影响文件：`android-tv-app/tv-app/src/main/java/com/chee/videos/feature/tv/TvPosterWallScreen.kt`（常量 + spec + 网格 minSize + 卡片重写）、`android-tv-app/tv-app/src/test/java/com/chee/videos/feature/tv/TvPosterWallFocusLayoutSpecTest.kt`（测试改名 `flush dark title bar`→`gradient scrim title` + 断言改 scrim/本地 token/中性 shadow；新增 shared-element 绑图片节点测试、6 列密度测试、顶行溢出护栏）、`android-tv-app/tv-app/build.gradle.kts`（版本 0.1.127/127→0.1.128/128）、`CONTEXT.md`（第 721 行契约校准 25%→45% + 补本地 scale token/中性落影/shared-element 绑图片节点/占位卡不叠标题契约）、`plan.md`。**不碰** `TvFocus.kt`（共享 token 与修饰器签名不动）、**不碰** release APK（`android-tv-app/tv-app/release/`，锁 121）与 `internal/services/tv_apk_test.go` 断言（锁 121）——遵循 CONTEXT.md「不重建 release 固件、不碰 Go 断言」口径，APK 断言与 build.gradle 版本号故意脱钩。
- 验证：`cd android-tv-app && ./gradlew :tv-app:compileDebugKotlin` BUILD SUCCESSFUL；`:tv-app:testDebugUnitTest --tests TvPosterWallFocusLayoutSpecTest --tests TvFocusSpecTest` BUILD SUCCESSFUL（全绿）。待办：①完整 `:tv-app:testDebugUnitTest` + `:tv-app:assembleDebug`；②`git diff --check` + 乱码扫描；③子代理独立复审修复后 diff；④真机手测：6 列密度、纯缩放 1.08 + 中性落影观感、渐变标题两行可读性、电视剧卡 shared-element 过渡不带标题飞入、占位卡单标签、顶行聚焦圆角不裁。
...
- 验证：`go test ./internal/handlers ./internal/queue ./internal/repository -run 'Ed2k|ed2k|LifecycleMigration' -count=1` 通过；`cd admin-web && npm run test -- src/views/toolbox.helpers.spec.js src/views/ToolboxEd2kDownload.spec.js src/api/admin.spec.js` 通过；`cd admin-web && npm run build` 通过（仅既有 chunk size warning）；`go build ./...` 通过；`go vet ./...` 通过；`git diff --check` 通过；`rg -n $'\uFFFD' ...` 无输出；最终独立复审 `no findings`。
...
## 2026-07-01 20:00 +0800
- 进度：继续收口 ED2K 下载工作台最终独立复审遗留问题，当前只处理三条明确缺陷：①`queued` 删除不能物理删掉“从 files_cleaned 重下出来的历史任务”，应改为撤销本次重下并恢复回 `files_cleaned`；②`queued` 删除必须同步删除 asynq 中同 `TaskID` 的非 active job，避免孤儿 job；③创建弹窗自动关闭判定不能再被历史失效结果永久卡住。计划先补红灯测试，再做最小实现，最后重新跑定向验证并再次独立复审。`CONTEXT.md` 现有无关 TV 海报墙改动继续排除，不纳入本轮提交。
- 影响文件：预计涉及 `internal/handlers/admin_ed2k_download.go`、`internal/handlers/admin_ed2k_download_test.go`、`internal/repository/ed2k_download_repository.go`、`admin-web/src/views/toolbox.helpers.js`、`admin-web/src/views/toolbox.helpers.spec.js`、`admin-web/src/views/ToolboxEd2kDownload.vue`、`plan.md`
- 验证：待执行 `go test ./internal/handlers ./internal/queue ./internal/repository -run 'Ed2k|ed2k' -count=1`、`cd admin-web && npm run test -- src/views/toolbox.helpers.spec.js src/views/ToolboxEd2kDownload.spec.js src/api/admin.spec.js`、`cd admin-web && npm run build`、`go build ./...`、`go vet ./...`、`git diff --check`、乱码扫描。
...
## 2026-07-01 18:44 +0800
- 进度：继续按独立复审收口 ED2K 下载工作台剩余阻塞项。本轮重点锁定三条状态机问题：①`running -> canceling -> cancelled` 取消链路必须改成条件更新，避免管理员取消反向覆写 worker 已落成的 `completed/failed`；②取消失败与残留清理失败必须拆语义，只有“引擎已停但清理失败”的 `cancelled` 才允许“重试清理”，引擎侧取消失败要留在 `canceling` 并允许继续重试取消；③`files_cleaned -> retry` 的写库必须限制当前仍为 `files_cleaned`，否则 worker 抢先进入 `running` 后会被 API 线程反写回 `queued`。同时会补齐 `cancelled` 补做清理成功后的独立清理时间展示。
- 影响文件：预计涉及 `internal/handlers/admin_ed2k_download.go`、`internal/handlers/admin_ed2k_download_test.go`、`internal/repository/ed2k_download_repository.go`、`internal/queue/ed2k_download.go`、`admin-web/src/views/ToolboxEd2kDownload.vue`、`admin-web/src/views/ToolboxEd2kDownload.spec.js`、`admin-web/src/api/admin.spec.js`、`CONTEXT.md`、`plan.md`
- 验证：待执行 `gofmt -w internal/handlers/admin_ed2k_download.go internal/handlers/admin_ed2k_download_test.go internal/repository/ed2k_download_repository.go admin-web/src/views/ToolboxEd2kDownload.vue`、`go test ./internal/handlers ./internal/queue ./internal/repository -run 'Ed2k|ed2k' -count=1`、`cd admin-web && npm run test -- src/views/toolbox.helpers.spec.js src/views/ToolboxEd2kDownload.spec.js src/api/admin.spec.js`、`cd admin-web && npm run build`、`go build ./...`、`go vet ./...`、`git diff --check`、乱码扫描。

## 2026-07-01 16:50 +0800
- 进度：继续复审后又补掉两条真实问题。其一，`files_cleaned -> retry` 先入队再改库虽然修复了“数据库 queued 但队列无 job”，但仍会在“入队成功、改库失败”时留下反向悬空：worker 读到 job 时因数据库仍是 `files_cleaned` 直接跳过，导致这次重试被静默吞掉。本轮给 `queue.Enqueuer` 增加 `DeleteEd2kDownloadTask`，在 `retryEd2kDownloadTask` 的入队成功但 `RequeueFilesCleanedEd2kDownloadTask` 失败时立即撤销刚入的 job，并补回归测试锁住。其二，`clean-files` 之前只校验结果条目拼接后的相对路径文本，若任务目录内某一级父目录本身是指向外部目录的 symlink，仍可能把“看似在任务目录内”的删除落到外部路径；本轮补了对结果父目录 realpath 必须仍落在任务目录 realpath 下的校验，并补测试覆盖目录级 symlink 绕过。另同步收口详情页时间文案：`finished_at` 不再一律显示成“下载完成时间”，失败/取消态改按真实结束语义标注，`cleaned_at` 只在 `files_cleaned` 态展示，避免旧清理时间泄露到重新下载后的其它状态。
- 影响文件：`internal/queue/tasks.go`、`internal/handlers/admin_ed2k_download.go`、`internal/handlers/admin_ed2k_download_test.go`、`internal/handlers/admin_orphan_file_scan_test.go`、`internal/handlers/router.go`、`admin-web/src/views/ToolboxEd2kDownload.vue`、`plan.md`
- 验证：`go test ./internal/handlers ./internal/queue -run 'TestRetryEd2kDownloadTaskRollsBackEnqueuedJobWhenRequeueFails|TestCleanCompletedEd2kDownloadArtifactsRejectsSymlinkedParentDirectoryEscape' -count=1` 通过；`go test ./internal/handlers ./internal/queue ./internal/repository -run 'Ed2k|ed2k' -count=1` 通过；`cd admin-web && npm run test -- src/views/toolbox.helpers.spec.js src/views/ToolboxEd2kDownload.spec.js src/api/admin.spec.js` 通过；`cd admin-web && npm run build` 通过（仅既有 chunk size warning）；`go build ./...` 通过；`go vet ./...` 通过；`git diff --check` 通过；`rg -n $'\uFFFD' ...` 无输出。

## 2026-07-01 16:28 +0800
- 进度：继续对 ED2K 下载工作台做复审收口。当前重点不是扩需求，而是验证最后一批针对评审问题的修复是否真的闭环：①`files_cleaned` 重试改成先入队后改库后，是否还会留下“数据库排队但队列无 job”或反过来的悬空态；②`clean-files` 的相对路径、符号链接与任务目录边界是否足够严格，避免越权删除；③前端列表是否已完全回到后端排序语义，不再本地二次重排。无关 `CONTEXT.md` 旧改继续排除在本轮之外。
- 影响文件：`internal/handlers/admin_ed2k_download.go`、`internal/repository/ed2k_download_repository.go`、`internal/handlers/admin_ed2k_download_test.go`、`internal/queue/ed2k_download.go`、`admin-web/src/views/ToolboxEd2kDownload.vue`、`plan.md`
- 验证：待执行 `gofmt -w internal/handlers/admin_ed2k_download.go internal/repository/ed2k_download_repository.go internal/handlers/admin_ed2k_download_test.go internal/queue/ed2k_download.go`、`go test ./internal/handlers ./internal/queue ./internal/repository -run 'Ed2k|ed2k' -count=1`、`cd admin-web && npm run test -- src/views/toolbox.helpers.spec.js src/views/ToolboxEd2kDownload.spec.js src/api/admin.spec.js`、`cd admin-web && npm run build`、`go build ./...`、`go vet ./...`、`git diff --check`、乱码扫描。
...
## 2026-07-01 15:06 +0800
- 进度：开始按评审结果继续收口 ED2K 下载工作台，目标是边修边复审直到无阻塞问题。当前先锁定两条主线：①新建任务弹窗与批量提交结果必须改成逐行回显、部分成功留场、脏关闭确认与任务直达，不再提交后无条件清空关闭；②`running` 任务删除必须从伪 `deleted` 改成真实取消/清理语义，避免后台继续下载后把任务写回完成态。若现有表结构不足以承载 `cancelled` / `canceling` 等状态，本轮补最小前向兼容迁移与定向测试。
- 影响文件：预计涉及 `admin-web/src/views/ToolboxEd2kDownload.vue`、`admin-web/src/views/ToolboxEd2kDownload.spec.js`、`admin-web/src/views/toolbox.helpers.js`、`admin-web/src/views/toolbox.helpers.spec.js`、`internal/handlers/admin_ed2k_download.go`、`internal/handlers/admin_ed2k_download_test.go`、`internal/repository/ed2k_download_repository.go`、`internal/queue/ed2k_download.go`、`internal/queue/ed2k_download_test.go`、`internal/models/admin.go`、`migrations/0029_ed2k_download_tasks.*.sql` 或新增迁移、`internal/repository/migrations_test.go`、`CONTEXT.md`、`plan.md`
- 验证：待执行定向 Go / Vitest、`cd admin-web && npm run build`、`go test ./internal/handlers ./internal/queue ./internal/repository -run 'Ed2k|ed2k' -count=1`、必要时 `go test ./... -count=1`、`git diff --check`、乱码扫描。

## 2026-07-01 14:55 +0800
- 进度：完成 ED2K 下载工作台失败恢复语义收口。本轮移除了失败任务原任务重试链路：后端删除 `/api/v1/admin/ed2k-download/tasks/:id/retry` 路由与相关 handler/测试，前端移除“重试任务/重新排队”入口与 API 调用；worker 的 ED2K 下载失败改为在落 `failed` 后 `return nil`，切断 asynq 自动重投。失败恢复路径改为“删除旧任务后重新提交同一链接”：`failed` 删除现在会先对 aMule 发 `cancel <resource_hash>`（哈希不存在则视为已清空），再清理受管下载工作区残留，最后物理删除数据库记录，释放同资源重建入口。为此补了 ED2K 删除清理 helper、aMule 远控配置注入（复用既有 `AMULECMD_BIN` / `AMULE_REMOTE_*` 与 `ED2K_DOWNLOAD_ROOT` / `ED2K_DOWNLOAD_SUBDIR`），并更新 `CONTEXT.md`、部署文档和下载工作台文案。无关工作区改动未纳入。
- 影响文件：`CONTEXT.md`、`.env.example`、`docs/家用部署机.md`、`admin-web/src/api/admin.js`、`admin-web/src/views/ToolboxEd2kDownload.vue`、`admin-web/src/views/ToolboxEd2kDownload.spec.js`、`internal/config/config.go`、`internal/config/config_test.go`、`internal/handlers/router.go`、`internal/handlers/admin_ed2k_download.go`、`internal/handlers/admin_ed2k_download_test.go`、`internal/queue/ed2k_download.go`、`internal/queue/ed2k_download_test.go`、`main.go`、`plan.md`
- 验证：`go test ./internal/handlers -run 'TestRegisterIncludesEd2kDownloadRoutes|TestDeleteFailedEd2kDownloadArtifacts|TestRemoveFailedEd2kPathIgnoresNotExist|TestCancelFailedEd2kDownloadIgnoresHashNotFound'` 通过；`go test ./internal/queue -run 'TestHandleEd2kDownloadMarksFailedAndStopsAsynqRetry|TestCommandEd2kDownloadExecutorPassesTaskMetadataByEnv'` 通过；`go test ./internal/handlers ./internal/queue ./internal/config` 通过；`cd admin-web && npm run test -- src/views/ToolboxEd2kDownload.spec.js` 通过；`cd admin-web && npm run build` 通过（仅既有 chunk size warning）；`go build ./...` 通过；`go vet ./...` 通过；`git diff --check` 通过；`rg -n $'\uFFFD' ...` 无输出。

## 2026-07-01 14:18 +0800
- 进度：开始收口 ED2K 下载工作台失败恢复语义。已确认当前“去掉失败任务重试”不能只删按钮和 `/retry` API，因为现状里 `failed` 既不能删除、哈希去重又会继续挡住同资源重建，直接移除会把失败任务变成不可恢复死记录。进一步与用户确认后，本轮口径改为：失败任务不再支持原任务重试；管理员删除失败任务时，必须同时删除数据库记录与这次下载留下的本地残留文件，然后允许重新提交同一条 ED2K 链接创建新任务。部署机已复核 `amulecmd 3.0.0` 的 `status`、`show dl`、`help cancel` 可用，具备继续收口失败删除清理链路的前提。
- 影响文件：`internal/handlers/admin_ed2k_download.go`、`internal/handlers/admin_ed2k_download_test.go`、`internal/queue/ed2k_download.go`、`internal/queue/ed2k_download_test.go`、`internal/handlers/router.go`、`admin-web/src/api/admin.js`、`admin-web/src/views/ToolboxEd2kDownload.vue`、`admin-web/src/views/ToolboxEd2kDownload.spec.js`、`CONTEXT.md`、`plan.md`
- 验证：待执行定向 Go 测试、`go test ./internal/handlers ./internal/queue`、`cd admin-web && npm run test -- src/views/ToolboxEd2kDownload.spec.js`、`cd admin-web && npm run build`、`go build ./...`、`go vet ./...`、`git diff --check`、乱码扫描。
...
- 影响文件：`plan.md`（仅本进度条目；部署机 `.env`/脚本权限/launchd 运行态不进 git）。
- 待办：无。功能已上线，每日 01:17 本地时自动刷新 server.met 并重启 amuled；ED2K_SERVERLIST_URL 留空即禁用整功能。

## 2026-07-01 10:05 +0800
- 进度：完成 server.met 定时刷新编码 + 子代理独立评审 + 修复阻塞问题。三方案并行设计（一致性 A / 可靠性 B / 最小足迹 C）后主代理选优：取 A 的执行器 interface+struct 与 NewProcessor 参数、C 的 scheduler 接线（`scheduler.Start()` 非阻塞 + `srv.Run(mux)` 阻塞 + `defer scheduler.Shutdown()`，已对照 asynq v0.25.1 源码确认 `Server.Run` 与 `Scheduler.Run` 均内部 `waitForSignals`、故 scheduler 只能用 `Start`）、B 的 server.met 原子替换 + `0xE0` 魔数双校验 + 备份回滚；跳过判定走 A 风（repo 方法 `HasActiveEd2kDownloadTasks` 单条 `COUNT ... WHERE status IN ('queued','running')`，不单测跳过分支，与现有下载 handler 一致）。
- 影响（已落地）：`internal/config/config.go`+`config_test.go`（`ED2KServerlistURL`/`ED2KServerlistRefreshCron` 默认 `"17 1 * * *"`/`ED2KServerlistRefreshExecutable`/`ED2KServerlistRefreshTimeout` 四字段 + 2 测试）、`internal/repository/ed2k_download_repository.go`（`HasActiveEd2kDownloadTasks`）、`internal/queue/tasks.go`（`TypeEd2kServerlistRefresh` 常量 + `Ed2kServerlistRefreshPayload` + `Processor.ed2kServerlistExec` 字段 + `NewProcessor` 参数 + `Register`）、`internal/queue/ed2k_serverlist.go`（新：`Ed2kServerlistRefreshExecutor` 接口 + `CommandEd2kServerlistRefreshExecutor` + `Ed2kServerlistRefreshResult` + `HandleEd2kServerlistRefresh`）、`internal/queue/ed2k_serverlist_test.go`（新：执行器 5 测 + handler 4 测）、`main.go` `runWorker`（条件构建执行器 + scheduler 接线）、`scripts/ed2k-serverlist-refresh.sh`（新脚本）、`.env.example`、`docs/家用部署机.md`（第 3.1 节 6.1 小节）。CONTEXT.md 四术语与 `docs/adr/0015` 已在收口阶段落地。
- 验证：`go build ./...` 通过；`go vet ./...` 通过（修复 `append(os.Environ())` 无值 vet 警告 → 改 `os.Environ()`）；`go test ./internal/{config,queue,repository,utils,hashutil}` 全绿；脚本 `bash -n` 通过，stub amulecmd/launchctl 本地端到端实测：真实 URL happy path（1627 字节、`e009` 魔数、备份清理 0 残留、exit 0）+ 坏魔数 reject path（exit 68、原 server.met `e009` 保留、备份回滚）；乱码扫描无命中；`git diff --check` 通过；`gofmt -l` 干净。
- 评审与修复：子代理独立评审发现 1 个阻塞 + 3 个应修 + 2 个 nit。阻塞：scheduler 以 `nil` payload 入队，handler `json.Unmarshal(nil)` 返回 `unexpected end of JSON input` → 每次刷新未到在途检查就失败（功能不可用）。已修：handler 改 `len(task.Payload()) > 0` 才 unmarshal，并加 `TestHandleEd2kServerlistRefreshAcceptsNilPayload` 回归测试。应修：① 补 handler 级测试（nil payload / 空 `{}` payload / 坏 JSON / executor-nil 跳过）；② 脚本 post-replace 魔数校验失败时回滚备份——原顺序先删备份后校验，调整为先校验、失败回滚、成功才删备份；③ 删除无人调用的 `EnqueueEd2kServerlistRefresh`（其 `{}` payload 与 scheduler 的 `nil` 不对称，正是它掩盖了阻塞 bug）。nit：gofmt 两文件、env 注入注释改为说明“刻意与下载执行器不同”。已全部修复并复测通过。
- 待办（部署机运行态，不进 git）：部署机 `.env` 写入 `ED2K_SERVERLIST_URL`/`ED2K_SERVERLIST_REFRESH_CRON`/`ED2K_SERVERLIST_REFRESH_EXECUTABLE` 三键，`chmod +x` 刷新脚本，重启 worker；首次 cron 触发后查 `~/Library/Logs/ai-video-server/worker.log` 确认 `ed2k serverlist refresh scheduler started` 与 `ed2k serverlist refresh completed`。

## 2026-06-30 11:20 +0800
- 进度：通过 `grill-with-docs` 收口“给 aMule server.met 做定时刷新”的需求与边界，未开始编码。已确认上游 `http://upd.emule-security.org/server.met` 是 eMule `server.met` 二进制格式（首字节 `0xE0`+版本号，aMule 原生可读），eMule-Security.org 每日 16:02 UTC 更新一次，其 FAQ 推荐 eMule 内置“Update server.met from URL”或“Auto-update server list at startup”。已确定方案形态：在 Go worker 内新增一个 asynq 周期任务（`asynq.Scheduler` 注册 cron，默认 `"17 1 * * *"` 本地时，`ED2K_SERVERLIST_REFRESH_CRON` 可配）驱动刷新；实际“下载 server.met 落盘 `~/.aMule/server.met` + 重启 amuled”由新增独立执行器脚本 `scripts/ed2k-serverlist-refresh.sh` 完成，Go 侧只负责定时入队 + 在途任务跳过判定 + 记日志，不写 `~/.aMule/server.met`、不调 `amulecmd`，延续 [[ED2K aMule 执行器契约]] 边界。生效方式选“重启 amuled”（`launchctl kickstart -k gui/$(id -u)/com.aivideo.amuled`）而非 `amulecmd` EC reload，因部署机 amulecmd 动作命令不可靠。刷新前查 `ed2k_download_tasks` 有 `status IN ('queued','running')` 则跳过本轮，保护在途下载。结果只进 worker 日志，不落 DB、不进管理端界面。配置三键：`ED2K_SERVERLIST_URL`（留空即禁用整功能）、`ED2K_SERVERLIST_REFRESH_CRON`、`ED2K_SERVERLIST_REFRESH_EXECUTABLE`。
- 影响（计划，未落地）：`CONTEXT.md`（新增 [[ED2K 服务器列表刷新]]/[[ED2K 服务器列表刷新归执行器脚本]]/[[ED2K 服务器列表刷新重启 amuled 生效]]/[[ED2K 服务器列表刷新在途任务跳过]] 四条术语）、`docs/adr/0015-ed2k-serverlist-periodic-refresh.md`（新增）、`plan.md`；待编码影响：`internal/config/config.go`+`config_test.go`（三键）、`internal/queue/tasks.go`+`ed2k_serverlist_refresh.go`+对应 `_test.go`（新任务类型 `ed2k:serverlist:refresh`、Enqueuer、Processor handler 含跳过判定、CommandEd2kServerlistRefreshExecutor）、`main.go` `runWorker`（按配置启动 asynq.Scheduler）、`scripts/ed2k-serverlist-refresh.sh`（新脚本）、`.env.example`、`docs/家用部署机.md`（部署机 `.env` 与 amuled 重启约定补充）。
- 验证：暂无（仅收口阶段）。上游 server.met 已实测：`curl -I` 返回 200、`Content-Length: 1637`、`Last-Modified: Mon, 29 Jun 2026 16:02:20 GMT`，首字节 `e0 09`；eMule-Security.org `/api/faq` 确认刷新 URL 与“Auto-update server list at startup”说明。CONTEXT/ADR/plan 已落，乱码与 diff 检查待编码完成后统一跑。

## 2026-06-30 10:30 +0800
- 进度：定位并修复 ED2K 下载工作台“server.met 已配置但 amuled 不连接”的根因。系统化排查确认部署机 MacPorts `amule @2021.12.05`（aMule 2.3.3 snapshot，wxWidgets 3.0.5）在 macOS daemon 模式下事件循环缺陷——`CTimerThread` 在 fire timer 事件（`CTimerEvent::Clone`），但主事件循环不派发，Autoconnect 连接逻辑从未执行：`amuled` 启动后日志停在 `OnInit - starting timer`，2 小时零出站 TCP/UDP、零服务器连接，`amulecmd` 的 `status`/`show dl`/`connect` 全部静默超时（只有 `help` 返回）。已用单一变量实验排除网络/防火墙/`SafeServerConnect`/前台 vs full-daemon/`-o` 日志抑制等假设。修复方案是换用官方 aMule 3.0.0 macOS dmg（universal2，含 arm64）的 `aMule.app` bundle（wxWidgets 3.3.2，事件循环正常）：部署机已下载 dmg、把完整 bundle 放到 `~/deploy/aMule.app`，`.env` 的 `AMULED_BIN`/`AMULECMD_BIN`/`ED2K_BIN` 与 launchd plist `ProgramArguments` 都改为指向 bundle 内 `Contents/MacOS/` 二进制（`amuled -c ~/.aMule -d -o`）。隔离测试（独立配置目录）4 秒连上 `ed2k-rust` 服务器拿到 Low ID、连上 Kad，`amulecmd status` 正常返回，`ed2k` helper 提交链接返回 `Link successfully queued.`，对比 2.3.3 完全验证修复。
- 阻塞：生产 `~/.aMule/amule.conf` 的 `IncomingDir` 指向外挂盘 `/Volumes/large/ai-video-server/storage/ed2k-downloads`，3.0.0 `amuled` 作为新 binary identity 被 macOS TCC 拦截可移动卷访问，主线程卡在 `CSharedFileList::Reload → wxDir::Open → open()`，日志停在“没有找到 part 文件”后无推进。纯 SSH 无法授权（直接写 TCC.db 被 tccd 忽略），需在部署机本机 GUI 给 `~/deploy/aMule.app` 授可移动卷访问。当前 amuled 已 bootout 停放，`IncomingDir` 已改回外盘契约路径，待 GUI 授权后重新 `launchctl bootstrap` 即可。临时改本地盘 `IncomingDir` 验证过 3.0.0 能正常连接与提交，闭环可用性已确认，仅差外盘授权这一步落地。
- 影响文件：`docs/家用部署机.md`（前置依赖段 + 第 3.1 节重写为 dmg bundle 部署 + 故障速查新增 ED2K 排查行）、`docs/examples/com.aivideo.amuled.plist`（`ProgramArguments` 改为 bundle amuled + `-c`/`-d`/`-o`）、`.env.example`（三个 bin 路径注释）、`CONTEXT.md`（[[ED2K aMule 部署前提]]/[[ED2K aMule 远控参数契约]]/[[ED2K aMule 链接提交契约]] 改写，新增 [[ED2K aMule 3.0.0 事件循环修复]] 与 [[ED2K aMule 外盘 TCC 授权前提]] 两条术语）、`plan.md`；部署机运行态（不进 git）：`~/deploy/aMule.app` bundle、`~/deploy/ai-video-server/.env` 三个 bin 路径、`~/Library/LaunchAgents/com.aivideo.amuled.plist` `ProgramArguments`、`~/.aMule/amule.conf`；部署机本地备份：`.env.pre3upgrade`、`com.aivideo.amuled.plist.pre3upgrade`、`~/.aMule/amule.conf.pre3upgrade`。
- 验证：部署机隔离实例 `amuled 3.0.0 -c /tmp/amule3-test` 4 秒连上 ed2k-rust + Kad，`amulecmd status` 返回 `Connected to ed2k-rust with LowID`，`ed2k` helper 返回 `Link successfully queued.`；生产 launchd 实例在本地盘 `IncomingDir` 下同样连接成功（出站 TCP ESTABLISHED、用户编号 124956）；外盘 `IncomingDir` 下 `sample` 确认卡在 `wxDir::Open → __open_nocancel`（TCC 阻塞，待 GUI 授权）；TCC 数据库确认 `/opt/local/bin/amuled`（2.3.3）有 `kTCCServiceSystemPolicyRemovableVolumes` 授权而 3.0.0 bundle 路径无。仓库文档 `rg` 乱码扫描 + `git diff --check` 通过。未改 Go / 前端代码，未跑 Go 测试（无代码变更）。

## 2026-06-29 15:00 +0800
- 进度：通过 `$grill-with-docs` 对未完成的 ED2K 下载工作台做了一次整体审查，只收口术语与边界，不做实现。结论按四条线落地：①“已下载但文件已清理”定稿为显式 status 枚举值 `files_cleaned`，由 `completed` 任务上的“删除暂存文件”动作触发；②清理归 Go handler 直接做 fs 操作（B1）——`os.Readlink` 解析符号链接、校验真路径在 `ED2K_DOWNLOAD_ROOT` 下、`os.Remove` 删 aMule Incoming 真文件并清 `OUTPUT_DIR`，并给 Go config 新增 `ED2K_DOWNLOAD_ROOT`；③清理范围同时删符号链接与 aMule 真文件（彻底回收磁盘，b 原样），代价是重试依赖 aMule 重下、可能落 `failed`（a1）；④`files_cleaned` 可重试不可新建（a），去重侧仍按已下载挡新建、重试侧放开同任务重下。审查中另发现两处代码隐患并定稿收口口径：asynq 入队缺 `asynq.TaskID` 导致可能并发下载（定稿入队 TaskID 一对一绑定 + 失败 `return nil` 切断 asynq 自动重投，P）；手动重试 handler 缺 `EnqueueEd2kDownload`、是假动作（定稿重试必须重新入队，TaskID 冲突时返回“任务已在执行中”）。导入（I1/I2/I3）整体搁置不做。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档变更，未运行构建/测试；`git diff --check` 与乱码扫描待跑。

脚本现在通过 `ED2K_BIN` 使用 aMule 自带 `ed2k` helper 提交链接，保留 `amulecmd help` 作为远控就绪检查，不再调用当前部署机上会超时的 `amulecmd Add` / `Show DL` / `Status`。部署机短超时实测使用用户提供链接运行脚本，已进入等待下载阶段并按 `ED2K_WAIT_TIMEOUT_SECONDS=10` 正常返回 `ed2k download timed out after 10 seconds`，说明执行器不再卡在命令提交；同时发现 aMule 日志仍显示 `0 servers in server.met found`，当前剩余阻塞是 aMule 网络/服务器列表未配置，尚未形成真实下载文件。
...
## 2026-06-27 11:55 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“queued 阶段的详情页是否也应固定显示预期文件信息区块”的边界，不做实现。已确认即使任务尚未开始实际下载、也还没有任何落盘快照，详情页仍应保留独立的“预期文件信息”区块，避免页面只剩状态标签；相应补强 `ED2K 进行中预期文件信息` 术语。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-27 11:50 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“任务详情是否应在所有可见状态下持续保留可复制来源标识”的边界，不做实现。已确认只要任务仍保留详情页，原始 ED2K 链接与资源哈希就应始终可见可复制，不因 `queued`、`running`、`completed`、`failed` 或“已下载但文件已清理”而隐藏；相应将局部术语收口为 `ED2K 任务详情来源标识常驻`。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-27 11:41 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“任务失败后列表摘要是否应从预期信息切换为失败残留摘要”的边界，不做实现。已确认失败阶段若已有残留快照，列表摘要应优先切换为失败残留摘要，不再沿用旧的预期文件信息；相应将 `ED2K failed 后列表摘要优先失败残留` 写入 `CONTEXT.md`。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-27 11:30 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“任务完成后列表摘要是否应从预期信息切换为实际结果”的边界，不做实现。已确认进入 `completed` 后，列表摘要应切换为实际落盘结果摘要，不再沿用旧的预期文件信息；相应将 `ED2K completed 后列表摘要切换为实际结果` 写入 `CONTEXT.md`。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。
...
## 2026-06-27 11:17 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“任务列表默认是否把进行中任务置顶”的边界，不做实现。已确认下载页是监控面而不是归档页，`queued` / `running` 应置顶并按最近状态更新时间倒序，已结束任务排在后面；相应将 `ED2K 任务列表进行中置顶` 写入 `CONTEXT.md`。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-27 11:05 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“任务列表是否只展示简洁文件摘要、完整清单留给详情”的边界，不做实现。已确认列表层只保留轻量文件摘要，多文件任务不在列表里展开完整清单；相应将 `ED2K 任务列表文件摘要优先` 写入 `CONTEXT.md`。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-27 10:36 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“列表和详情里是否必须区分预期文件信息、过程快照和最终结果”这一显示语义，不做实现。已确认三类文件信息来源必须分层可辨，不能混成一个模糊文件列表；相应将 `ED2K 文件信息来源必须分层可辨` 写入 `CONTEXT.md`。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。
...
## 2026-06-27 09:35 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“running 取消后补做清理成功是否改写任务生命周期”的边界，不做实现。已确认这类任务应继续保持 `cancelled` 历史，只去掉清理失败语义并另记清理完成时间；相应将 `ED2K running 取消补做清理成功仍保持 cancelled` 写入 `CONTEXT.md`。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-27 09:33 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“running 取消后若只剩清理失败，是否要给管理员独立补救入口”的边界，不做实现。已确认任务一旦落成 `cancelled`，详情页仍应提供独立的“重试清理”动作，用来恢复残留文件清理，而不是重走取消流程；相应将 `ED2K running 取消清理失败支持重试清理` 写入 `CONTEXT.md`。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-27 09:31 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“running 取消时引擎已停但清理失败如何落态”的边界，不做实现。已确认这类任务不应卡在“取消中”，而应落成 `cancelled` 历史任务，并单独保留清理失败信息；相应将 `ED2K running 取消清理失败仍落成 cancelled` 写入 `CONTEXT.md`。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-27 09:28 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“running 取消是否需要明确过渡态”的状态边界，不做实现。已确认 running 取消不是瞬时动作，需先进入取消中过渡态，直到引擎确认停止且部分文件清理完成；相应将 `ED2K running 取消中的过渡态` 写入 `CONTEXT.md`。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-27 09:23 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“running 取消时是否清理部分下载文件”的边界，不做实现。已确认 running 取消后历史任务保留，但部分下载文件应同步清理，避免后续新任务与旧半成品互相污染；相应将 `ED2K running 取消同时清理部分下载文件` 写入 `CONTEXT.md`。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-27 09:21 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“running 取消后的历史判定与后续重提”边界，不做实现。已确认 running 取消既然落成 cancelled 历史任务，就不计入已下载判定，并允许同一链接后续按新任务重新提交；相应将 `ED2K running 取消不计入已下载且允许重提` 写入 `CONTEXT.md`。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-27 09:20 +0800
- 进度：通过 `$grill-with-docs` 收口 TV app 左侧菜单聚焦视觉方案，不做实现。目标为「聚焦时切换颜色而非 scale，让用户明显知道焦点在哪」。已逐决策点确认：①聚焦与已选中(实色金 Accent 填充)用**描边区分**——聚焦 = 暖金 `AccentStrong` 描边 + 背景提亮，选中 = 金底填充不动，金边=焦点在哪、金底=当前页在哪，两层可叠加；②聚焦描边取**金边 + 背景提亮**（背景 `Surface`→`SurfaceElevated` + `1.dp` `AccentStrong` `BorderStroke`，对齐选集轨 `1.dp` 边框先例）；③去掉 scale 时一并**去掉按下下沉 + CONFIRM 触觉**，对齐选集轨「聚焦集只用静态高亮」；④实现**仅内联**在 `TvHomeSideMenuButton`，不抽共享修饰器、不动选集轨；⑤`IPTV`/`短视频` action-only 永不被选中，故只有纯聚焦态（深色提亮 + 金边），简化分辨；⑥这是 [[TV 焦点反馈只缩放]] 全局规则的**例外**，以新术语 `TV 侧边菜单聚焦高亮` 点明，原全局条不动，回链 [[选集轨双态高亮]]；⑦**加红灯源码结构断言**（侧边菜单不再含 `tvFocusableScaleOnly`、改用 `.focusable()`、含 `onFocusChanged` + `BorderStroke` + `SurfaceElevated`）并 bump `versionCode` 125→126 / `versionName` 0.1.125→0.1.126，不重建 release 固件、不碰 `tv_apk_test.go`（固件与 Go 断言仍停在 121，脱节为已知接受项）。已将 `TV 侧边菜单聚焦高亮` 术语写入 `CONTEXT.md`。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。
...
## 2026-06-27 09:15 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“queued 删除后是否立即释放哈希占用”的边界，不做实现。已确认 queued 任务既然取消就等同永久删除，它占用的资源哈希也必须立即释放，使同一链接后续可按全新任务重新提交；相应将 `ED2K queued 删除立即释放哈希占用` 写入 `CONTEXT.md`。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-27 09:14 +0800
- 进度：补记此前已确认但尚未写入词汇表的 queued 取消语义，不做实现。已确认 queued 任务的取消动作在当前阶段等同删除并永久去掉，不保留长期可见的 cancelled 历史；相应将 `ED2K queued 取消即永久删除` 写入 `CONTEXT.md`，使其与刚确认的“queued 状态允许主动取消”保持一致。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-27 09:09 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“引擎 queued 状态是否允许主动取消”的操作边界，不做实现。已确认 queued 状态允许管理员主动取消；相应将 `ED2K queued 状态允许主动取消` 写入 `CONTEXT.md`，让状态区分与操作边界保持一致。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-27 09:08 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“引擎 queued 与 running 是否合并展示”的状态边界，不做实现。已确认二者必须明确区分：queued 只是进入引擎未开始传输，running 才是实际下载；相应将 `ED2K 引擎 queued 与 running 明确区分` 写入 `CONTEXT.md`，让状态展示继续忠实服从引擎回报。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-27 09:03 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“建任务前失败是否影响后续历史判断”，不做实现。已确认这类失败没有任务实体，因此后续再提交时应按全新链接处理，不参与去重、已下载判定或任务跳转；相应将 `ED2K 建任务前失败不影响后续去重` 写入 `CONTEXT.md`。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-27 09:01 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“建任务前失败”和“已建任务但提交引擎失败”的边界，不做实现。已确认前者不生成任务实体、没有任务定位，必须与后者严格区分；相应将 `ED2K 建任务前失败不生成任务实体` 写入 `CONTEXT.md`，把提交回执里的两类失败落点彻底拆开。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 19:08 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“多链接提交中某一行提交引擎失败时的回执落点”，不做实现。已确认这类失败行应直接返回对应失败任务的定位，便于管理员立刻进入详情查看失败原因并重试；相应将 `ED2K 提交引擎失败行返回失败任务定位` 写入 `CONTEXT.md`，把逐行提交回执与失败恢复链路接通。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 19:07 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“手动重试待同步后首次回报已完成”的状态收敛，不做实现。已确认这类任务可以直接从待同步过渡态切到已完成，不必补演一个伪造的 running；相应将 `ED2K 手动重试待同步可直达完成` 写入 `CONTEXT.md`，让状态机继续完全服从引擎真实回报。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 19:03 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“手动重试待同步后最终未被引擎接纳”的回退语义，不做实现。已确认这类任务应回退到原失败语义，并把这次重试累计为又一次失败；相应将 `ED2K 手动重试待同步落空回退失败` 写入 `CONTEXT.md`，让过渡态与失败累计逻辑保持一致。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 19:00 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“手动重试送达引擎后但尚未拿到新状态”的可见状态，不做实现。已确认此时应先显示等待引擎状态同步的过渡态，而不是直接伪装成 `running`；相应将 `ED2K 手动重试后的待同步过渡态` 写入 `CONTEXT.md`，保持失败恢复状态机继续以引擎回报为准。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 18:51 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“提交引擎失败后的重试策略”，不做实现。已确认首期只保留管理员手动重试，不做后台自动重试；相应将 `ED2K 提交引擎失败首期只手动重试` 写入 `CONTEXT.md`，让失败恢复路径继续保持可见、可控、可解释。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 18:49 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“原任务重试成功后如何闭环”，不做实现。已确认这类任务后续某次手动重试如果成功，也应继续沿用原任务记录并保留之前的失败历史；相应将 `ED2K 原任务重试成功回归同一任务` 写入 `CONTEXT.md`，让失败、重试与成功继续收敛在同一任务上下文中。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 18:40 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“原任务重试失败后如何累计历史”，不做实现。已确认同一条失败任务的后续多次手动重试若继续失败，也应累计在原任务里，而不是每次失败再派生新任务；相应将 `ED2K 原任务重试累计失败次数` 写入 `CONTEXT.md`，让失败恢复闭环继续收敛在同一任务上下文中。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 18:09 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“提交外部引擎失败后的恢复动作”，不做实现。已确认这类失败任务在详情页应允许直接重试提交到引擎，而不是回到粘贴入口新建第二条任务；相应将 `ED2K 提交引擎失败支持原任务重试` 写入 `CONTEXT.md`，让失败记录与恢复动作保持在同一任务上下文中。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 18:06 +0800
- 进度：继续围绕 ED2K 下载一期主链路，通过 `$grill-with-docs` 收口“任务已创建但提交引擎失败”时的落点，不做实现。已确认项目侧一旦接收并创建了下载任务，即使提交外部引擎失败，也应保留为失败任务记录；相应将 `ED2K 提交引擎失败仍留失败任务` 写入 `CONTEXT.md`，保证失败尝试在列表、排查和后续重试上都有明确对象。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 18:04 +0800
- 进度：按你的提醒把讨论焦点拉回 ED2K 下载一期主链路，继续通过 `$grill-with-docs` 收口“任务何时显示为下载中”的状态来源，不做实现。已确认只要外部引擎已进入 `running`，列表与详情就应优先显示为下载中，而不是等待项目侧同步补齐；相应将 `ED2K 引擎 running 优先显示` 写入 `CONTEXT.md`，保持状态来源继续以引擎为准。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 18:01 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K “已下载但文件已清理”在详情页中的来源追溯能力，不做实现。已确认这类历史任务仍应展示原始 ED2K 链接或可复制的资源标识，以支撑去重命中解释和人工核对；相应将 `ED2K 已下载文件已清理来源标识保留` 写入 `CONTEXT.md`。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 17:53 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K “已下载但文件已清理”在详情页中的时间展示，不做实现。已确认这类任务详情应同时展示下载完成时间与文件清理时间，而不是只保留其一；相应将 `ED2K 已下载文件已清理详情双时间展示` 写入 `CONTEXT.md`，让前面已分离的时间语义在界面层也保持可见。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 17:52 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K 暂存文件清理对时间字段的影响，不做实现。已确认清理动作不应覆盖任务原本的下载完成时间；如果要展示清理时间，也必须作为独立字段存在；相应将 `ED2K 清理时间不覆盖完成时间` 写入 `CONTEXT.md`，保证排序与历史时间线不被后续管理动作污染。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 17:48 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K “已下载但文件已清理”在任务列表中的排序语义，不做实现。已确认这类历史任务仍应按原完成时间参与排序，而不是因当前不可操作被自动沉底；相应将 `ED2K 已下载文件已清理排序沿用原完成时间` 写入 `CONTEXT.md`，让默认可见、可搜索和排序逻辑保持一致。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 17:46 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K “已下载但文件已清理”在任务搜索中的可见性，不做实现。已确认这类历史任务仍应参与关键字搜索结果，因为它们持续影响去重与已下载判定；相应将 `ED2K 已下载文件已清理搜索可命中` 写入 `CONTEXT.md`，避免管理员按文件名追查历史任务时再次出现搜索断层。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 17:43 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K “已下载但文件已清理”在默认任务列表中的可见性，不做实现。已确认这类任务虽然当前不可操作，但仍应在默认视图里继续显示，因为它们会影响去重与已下载判定；相应将 `ED2K 已下载文件已清理默认视图可见` 写入 `CONTEXT.md`，避免“提交时可命中、列表里却看不见”的断裂体验。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 17:40 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K “已下载但文件已清理”时的导入动作，不做实现。已确认这类任务不应继续保留“导入”入口，因为当前文件已不存在；相应将 `ED2K 已下载文件已清理导入动作关闭` 写入 `CONTEXT.md`，让详情状态与导入可操作性保持一致。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 17:37 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K “已下载但文件已清理”时的详情动作，不做实现。已确认这类任务不应继续保留“删除暂存文件”这个下载侧操作，最多只能显示为不可用；相应将 `ED2K 已下载文件已清理删除动作关闭` 写入 `CONTEXT.md`，让任务状态与可执行动作保持一致。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 17:33 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K “已下载但文件已清理”在任务列表中的归类，不做实现。已确认这类任务不应继续混入普通已完成筛选，而要有单独可筛选状态；相应将 `ED2K 已下载文件已清理列表筛选态` 写入 `CONTEXT.md`，让列表筛选与详情状态保持一致。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 17:27 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K “已下载但暂存文件已删”在任务详情中的显示语义，不做实现。已确认这类历史任务详情页不能继续显示成普通已完成，而应明确区分为“已下载但文件已清理”之类的独立状态；相应将 `ED2K 已下载文件已清理详情态` 写入 `CONTEXT.md`，避免管理员误判为仍可直接导入。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 17:24 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K “已下载但暂存文件已删”时的判定，不做实现。已确认删除暂存文件不会把同一资源重新视为未下载；相应将 `ED2K 删除暂存文件不改已下载判定` 写入 `CONTEXT.md`，保持“已下载”语义继续建立在历史任务记录而不是文件现存状态之上。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 17:23 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K “已下载且禁止重建”时的提交回执，不做实现。已确认命中历史已下载记录时，提交结果除了提示“已下载”，还必须返回对应历史任务的标识或跳转目标；相应将 `ED2K 已下载命中返回历史任务定位` 写入 `CONTEXT.md`，保证管理员能直接进入既有任务详情，而不是停在抽象提示上。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 17:19 +0800
- 进度：根据你刚收紧的新边界，继续通过 `$grill-with-docs` 回撤 ED2K 的“重建下载任务”语义，不做实现。已确认当前阶段命中历史已下载记录时不允许重新创建下载任务，只提示该资源已下载并引导查看既有任务；相应删除或改写了 `ED2K 已下载重建确认`、`ED2K 已下载行二次确认后单独重提`、`ED2K 待确认重建可批量确认`、`ED2K 重建创建新任务记录`、`ED2K 重建任务新建暂存目录` 等已失效术语，并新增 `ED2K 已下载命中禁止重建`。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 17:16 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K 重建任务与暂存目录的关系，不做实现。已确认已下载资源一旦确认重建，新任务必须新建独立暂存目录，不能复用旧任务目录；相应将 `ED2K 重建任务新建暂存目录` 写入 `CONTEXT.md`，使其与“重建创建新任务记录”以及“任务目录隔离”保持一致。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

## 2026-06-26 17:14 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K “已下载后重建”与历史记录的关系，不做实现。已确认管理员一旦确认重建，系统必须创建全新的下载任务记录，而不是重置或复用旧任务；相应将 `ED2K 重建创建新任务记录` 写入 `CONTEXT.md`，使其与“进行中命中直接复用”“已下载需二次确认”两条边界保持一致。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

# 2026-06-26 17:05 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K “待确认重建”行的二次确认方式，不做实现。已确认如果一次提交里有多条行都命中“已下载需确认重建”，首期允许管理员多选后统一确认重建，而不是只能逐条确认；相应将 `ED2K 待确认重建可批量确认` 写入 `CONTEXT.md`。这样既保留了“必须二次明确”的安全边界，也降低了多行重提时的操作成本。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

# 2026-06-26 17:02 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K “已下载需确认重建”行的提交流程，不做实现。已确认这类行在第一次提交时只标记为待确认，不会在同一个请求里顺带重下；只有管理员二次明确后，系统才单独重提这些行。相应将 `ED2K 已下载行二次确认后单独重提` 写入 `CONTEXT.md`，使其与 `ED2K 已下载重建确认` 的交互边界一致。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

# 2026-06-26 17:00 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K 多链接提交后的回执契约，不做实现。已确认首期提交结果必须按每条输入行返回结构化结果，而不是只给一个总提示；相应将 `ED2K 提交结果按输入行返回` 写入 `CONTEXT.md`。这和此前已确认的“每条链接独立决策”“允许部分成功”“无效行不阻断有效链接”三条边界保持一致。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

# 2026-06-26 16:56 +0800
- 进度：继续通过 `$grill-with-docs` 收口单次多链接提交中“有效链接 + 无效行”混合输入的处理，不做实现。已确认首期依然按部分成功处理：无效行单独标记并回显原因，但不阻断其它有效链接创建或复用任务；相应将 `ED2K 无效行不阻断有效链接` 写入 `CONTEXT.md`。这让本地工具已有的“识别无效行”心智和后端下载提交结果保持一致。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

# 2026-06-26 16:53 +0800
- 进度：继续通过 `$grill-with-docs` 收口无批次模型下的单次多链接提交结果，不做实现。已确认首期允许同一次 ED2K 提交出现部分成功：某些链接创建任务、某些链接复用进行中任务、某些链接提示已下载需确认重建、某些非法输入直接拒绝；相应将 `ED2K 单次提交允许部分成功` 写入 `CONTEXT.md`。这和“每条链接独立成为任务决策”的下载模型保持一致。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

# 2026-06-26 16:51 +0800
- 进度：继续通过 `$grill-with-docs` 收口取消批次后的 ED2K 页面主视图，不做实现。已确认首期只保留全量任务列表和单任务详情，不按“某次提交”长期分组展示；相应将 `ED2K 全量任务列表视图` 写入 `CONTEXT.md`。这让下载侧的长期管理对象继续保持为“单条任务”，而一次多链接提交只保留为创建入口动作。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。
...
# 2026-06-26 16:46 +0800
- 进度：继续根据你的新边界清理 ED2K 词汇表残留的“批次”术语，不做实现。已把 `ED2K 导入转交既有批次工作区`、`ED2K 导入生成独立入站批次快照` 改写为 `ED2K 导入转交既有入站工作区`、`ED2K 导入生成独立入站快照`，避免下载侧已经取消批次，而导入侧词条还在沿用旧对象名。当前 ED2K 讨论应统一围绕“单条下载任务”和“一次多链接提交动作”展开，不再引入持久化批次业务对象。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

# 2026-06-26 16:46 +0800
- 进度：根据你的新边界，继续通过 `$grill-with-docs` 回撤 ED2K 的“批次”概念，不做实现。已把“管理员一次可提交多条链接”改写为 `ED2K 多链接单次提交`，明确这只是一次创建多条独立下载任务的动作，而不是持久化的批次业务对象；同时删除或改写了 `ED2K 下载批次`、`ED2K 批次标题自动生成`、`ED2K 多链接批次标题默认取首条文件名`、`ED2K 批次目录隔离` 等批次化术语，收口为单任务目录隔离和任务详情视图。这样后续讨论会围绕“任务”而不是“批次”继续展开。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

# 2026-06-26 16:45 +0800
- 进度：继续通过 `$grill-with-docs` 收口多链接 ED2K 批次的自动标题规则，不做实现。已确认如果一次提交含多条有效链接，首期自动标题默认取第一条有效链接解析出的文件名，不拼接多个文件名，也不优先退化成“共 N 项”摘要；相应将 `ED2K 多链接批次标题默认取首条文件名` 写入 `CONTEXT.md`。同时已核对现有本地解析按原输入顺序保留有效链接，因此“首条有效链接”在当前语境里是稳定且可实现的。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

# 2026-06-26 16:43 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K 批次提交时的标题来源，不做实现。已确认首期允许管理员不填标题直接提交，系统应自动生成批次标题，并优先使用 ED2K 链接里解析出的文件名；相应将 `ED2K 批次标题自动生成` 写入 `CONTEXT.md`。同时已核对现有 `toolbox.helpers.js` 的 `getEd2kLinkLabel()` 已经能从 `ed2k://|file|...|` 结构中提取并解码文件名，这与该命名口径一致。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

# 2026-06-26 16:40 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K 进行中任务命中后的行为，不做实现。已确认如果同一资源哈希命中一条 `queued`、`running` 或等价进行中状态的历史任务，首期不允许重建，而是直接把管理员带到这条既有任务的详情或所属批次视图；相应将 `ED2K 进行中任务命中直接复用` 写入 `CONTEXT.md`。这样“能重提的是失败类，不能重提的是进行中类”的边界就完整了。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

# 2026-06-26 16:30 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K “已下载”提示所依据的历史状态边界，不做实现。已明确“历史任务存在”不包含 `failed`、`cancelled`、提交即拒绝等未真正完成下载闭环的记录；首期只有那些至少成功入队并实际进入过下载流程的历史任务，才算“该资源已下载过”。相应将 `ED2K 已下载历史状态边界` 写入 `CONTEXT.md`，替换掉过宽的“历史任务命中即视为已下载”表述。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

# 2026-06-26 16:27 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K 的“已下载”判定，不做实现。已按你的新边界修正为：只要同一资源哈希命中历史任务记录，就按“已下载过”处理，不再要求暂存文件当前仍存在；命中后仍需由管理员显式确认是否重新创建任务。相应将 `ED2K 历史任务命中即视为已下载` 写入 `CONTEXT.md`，并同步放宽 `ED2K 已下载重建确认` 的触发条件。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

# 2026-06-26 16:23 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K 下载链路的跨批次去重策略，不做实现。已确认同次提交原文去重之外，还应优先按 ED2K 链接自带的资源哈希做跨批次去重；未完成记录命中时不再并行新建任务，已完成记录命中时则提示管理员确认是否重建任务。相应将 `ED2K 资源哈希主去重键`、`ED2K 跨批次哈希防重`、`ED2K 已下载重建确认` 写入 `CONTEXT.md`。同时已核对现有本地 ED2K 解析测试样例本身就是 `ed2k://|file|name|size|hash|/` 结构，说明从链接文本中提取哈希在当前项目语境下是可行的。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

# 2026-06-26 16:19 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K 下载入口的首层防重，不做实现。已确认管理员同次粘贴多行 `ed2k://` 文本时，首期应按完全相同的原始链接文本去重，只创建一条子任务；相应将 `ED2K 同次提交原文去重` 写入 `CONTEXT.md`。同时已核对现有 `toolbox.helpers.js` 的本地解析只是逐行收集有效 ED2K 链接，当前没有任何去重逻辑。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

# 2026-06-26 16:16 +0800
- 进度：继续通过 `$grill-with-docs` 把讨论拉回 ED2K 链接下载本身，不做实现。已确认首期提交入口只支持管理员手动粘贴原始 `ed2k://` 文本，不扩展到 `.txt` 文件导入、网页抓取、磁力链接混输或其它来源形态；相应将 `ED2K 首期提交入口` 写入 `CONTEXT.md`。同时已核对现有 `ToolboxEd2k.vue` 只是浏览器端本地解析工具，当前不向后端提交内容。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

# 2026-06-26 16:11 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K 的“来源批次”和“入站批次”关系，不做实现。已确认 ED2K 页发起“导入”时，首期应生成一个新的入站批次快照，再转交给既有批次/文件工作区，而不是让 ED2K 批次和入站批次共享同一个工作区对象；相应将 `ED2K 导入生成独立入站批次快照` 写入 `CONTEXT.md`。同时已核对现有 `archive_import` 处理本身就是先从工作区复制到处理路径再落正式存储，这与“快照独立持有”思路一致。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

# 2026-06-26 15:54 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K 与既有入站工作区的边界，不做实现。已确认 ED2K 完成项的“导入”首期应转交到既有批次/文件式工作区，而不是在 ED2K 页面直接一键落库；相应将 `ED2K 导入转交既有批次工作区` 写入 `CONTEXT.md`。同时已核对现有 `archive_import` 批次删除会清理其自有工作区路径，这意味着后续若让 ED2K 与入站工作区直接共用同一目录，生命周期会互相耦合。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。

# 2026-06-26 15:47 +0800
- 进度：继续通过 `$grill-with-docs` 收口 ED2K 导入边界，不做实现。已确认 ED2K 完成项点击“导入”时应复用项目里既有的媒体入站流程，而不是新建一套 ED2K 专属入库编辑器；相应将 `ED2K 复用既有入站流程` 写入 `CONTEXT.md`。同时已核对当前代码面上现成的两类入口：一类是 `archive_import` 的批次/文件工作区流程，另一类是直接喂给 `UploadService` / `ImageService` 的本地文件落库流程。
- 影响文件：`CONTEXT.md`、`plan.md`
- 验证：文档探索阶段，无需构建。
...
## 2026-07-01 (grill-with-docs 收口，待实施)
- 进度：用户反馈 TV 海报墙（`TvPosterWallScreen`，用户口语称"书架墙"）卡片丑且太大。经 grill-with-docs 11 轮逐项拷问，定位两个根因并锁定 5 项设计决策。**根因①焦点木讷**：当前 `tvFocusableScaleOnly` 仅 1.04x 放大、无描边无光晕（[[TV 焦点反馈只缩放]] 全仓契约 + [[TV 海报墙海报卡]] 专属契约），3 米视距几乎无焦点区分度。**根因②卡片造型**：标题区是独立 74dp 深色色块（`TvPosterWallTitleBackground` alpha 0.90）硬压在海报下方，"胶布感"强；网格 `GridCells.Adaptive(minSize=170.dp)` 在用户 4K TV（density ~3.0，按 1080p 渲染）下仅 3–4 列，卡片过大。5 项决策定稿：①**焦点反馈 A+B**：放大 1.04→1.08 + 中性落影抬升 ~8dp elevation（`Modifier.shadow` 中性色 drop shadow，**非**暖金光晕），纯缩放契约保持；②**焦点安全区**维持 8dp（6 列窄卡下网格间距 16dp 兜底放大溢出+阴影扩散，不涨到 14dp）；③**标题区 D-轻**：移除独立标题色块，改渐变遮罩叠在海报底部 ~25%（透明→`AppChrome.Canvas` 0.65），标题压最底 + 细微文字阴影兜可读性；④**网格密度 F**：目标 6 列，`minSize` 170→~90dp（按真实可用宽度反推，density ~3.0 下 6×90+5×16≈620dp），到电视上校准；⑤**圆角 G-保持** 8dp 不动（沿用 `AppChrome.SurfaceShape`，零波及）。
- 实施约束（grill 中钉死）：(a) 放大倍率走 `TvFocusSafeSpec.posterFocusedScale` token（海报墙专属字段，改它不波及其他卡片），**不**动 `tvFocusableScaleOnly` helper 默认值（全仓共享）；(b) 落影 modifier 在 `TvPosterWallCard` 局部加（`tvFocusableScaleOnly` 不含 shadow，全仓 helper 不能塞海报墙专属逻辑）；(c) shared-element 过渡契约（[[TV 电视剧海报 shared-element 过渡]]）不破——D-轻 把渐变+标题移进 9:16 Box 后，过渡时它们随海报飞到详情页、到达后被覆盖，结构上 shared-element 仍绑同一 Box，实施时验证过渡中途不穿帮；(d) `TvPosterWallFocusLayoutSpecTest` 第 25 行测试 `poster wall cards use portrait artwork with flush dark title bar` 断言 `source.contains("TvPosterWallTitleBackground")`，D-轻 移除该色块后此断言红，且测试名描述旧契约——J-同步更新时该测试断言语义要从"存在独立深色标题条"改为"存在底部渐变遮罩 + 标题压遮罩"。
- 契约更新：CONTEXT.md 仅改 1 处——709 行 `TV 海报墙海报卡`，把"标题紧贴图片下方并使用深色底"改为"标题压在海报底部渐变遮罩之上"，把"聚焦态只允许放大"松绑为"聚焦态允许放大 + 中性落影抬升，仍不使用描边/暖金光晕"，并交叉引用 [[TV 焦点反馈只缩放]] 标明落影是允许例外。881 行全仓契约**不改**——其措辞始终是"暖金光晕"（带 `TvFocusGlowColor` 着色），中性落影本就不在其字面禁用范围，无需澄清。D-轻 渐变标题仍满足"深色底"字面契约（渐变到底部是 `AppChrome.Canvas` 深色），F 6列/minSize 未入契约，均不改 CONTEXT。
- 影响文件（待实施）：`android-tv-app/tv-app/src/main/java/com/chee/videos/feature/tv/TvPosterWallScreen.kt`（卡片重写：渐变遮罩标题区 + 落影 + minSize）、`android-tv-app/tv-app/src/main/java/com/chee/videos/core/ui/TvFocus.kt`（`posterFocusedScale 1.04→1.08` + 新增 `posterElevationDp: Float = 8f` token）、`android-tv-app/tv-app/src/test/java/com/chee/videos/feature/tv/TvPosterWallFocusLayoutSpecTest.kt`（J-同步更新：minSize 断言、移除 `TvPosterWallTitleBackground` 断言改为渐变遮罩断言、新增 elevation token 断言）、`android-tv-app/tv-app/src/test/java/com/chee/videos/feature/tv/TvPosterWallCardContentTest.kt`（若涉及标题内容构建则同步）、`android-tv-app/tv-app/build.gradle.kts`（versionCode+1 / versionName patch+1，按 AGENTS.md TV app 功能变更规则）、`CONTEXT.md`（已完成 709 行更新）、`plan.md`。
- 验证（待执行）：①`cd android-tv-app && ./gradlew --no-daemon :tv-app:testDebugUnitTest :tv-app:assembleDebug` 全绿；②`git diff --check` + 乱码扫描；③真机/模拟器手测：6 列密度、焦点 1.08 放大 + 落影浮起感、渐变标题可读性（浅色海报底部）、电视剧卡 shared-element 过渡不穿帮、首屏焦点仍落首项；④minSize 90dp 在目标 TV 上校准到恰好 6 列（可能需微调到 85–95dp）。
