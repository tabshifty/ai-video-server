...
- 验证：`git push deploy master` 返回 `deploy succeeded`；部署机 bare repo 已到 `15e3cc0`，远端 `curl http://127.0.0.1:8080/healthz` 返回 `{"status":"ok"}`。
...
- 验证：`npm test` 全量 40 测试文件 / 648 用例通过（含新增 9 个 helper 用例与 `precisionOpsRollout` 71 个回归用例）；`npm run build` 成功（仅既有 chunk 体积告警）。
...
## 2026-08-01 03:18 +0800
- 进度：完成 `docs/examples/115LocalNatManager` 实现链路审阅。示例由 Chrome 扩展把链接发给可配置地址的 Go 本地服务，后端调用 `POST /open/offline/add_task_urls`，定时轮询云下载任务，在 `status=2` 后取 `file_id`，再经文件详情的 `pick_code` 获取下载地址并拉取到本地。扩展界面、页面扫描和右键菜单当前只接受磁力链接或 HTTP 链接，未接入 ED2K；后端只检查 `urls` 非空，直接传递给 115，因而 ED2K 需由我们的入口显式校验与提交。示例没有首次 OAuth/PKCE，而是要求用户手工获取并粘贴令牌，所以不能用它证明 115 支持局域网回调。
- 风险：示例服务监听所有网卡，本地 API 无认证且 CORS 允许任意源；令牌在服务端 `0644` 明文落盘，扩展也持久化令牌。本地下载使用无超时的 `http.Get` 直接覆盖最终文件，不检查 HTTP 状态码，不支持 Range 断点续传，不校验文件大小或 SHA1，不限制 115 返回名称的目录穿越，读取中途错误时只要已收到数据就可能被标记为完成。任务与文件夹只处理第一页/前 100 项，跳过嵌套子目录；重启后 `downloading` 记录会永久跳过，并发信号量在周期检测中被替换，文件夹子任务又绕过并发限制。正式实现不复制这些行为。
- 影响文件：`plan.md`；`docs/examples/115LocalNatManager` 保持不变且不纳入根仓库提交，`CONTEXT.md` 无新的已决术语需追加。
- 验证：完成扩展、HTTP 路由、115 API 客户端、令牌管理、任务轮询与本地落盘源码静态审阅；`rg` 确认未实现 Range、文件完整性校验或临时文件原子替换。`go test ./...` 未进入编译，因示例缺少 `go.sum` 中 `github.com/gorilla/mux` 条目而失败；未为验证而改动示例依赖文件。

## 2026-08-01 03:06 +0800
- 进度：确认当前没有可用于 115 OAuth 回调的自有稳定域名。重新核对官方“概述”“接入流程”“授权码模式”“手机扫码授权 PKCE 模式”：官方没有说明局域网、内网 IP、`localhost` 或 `.local` 的接入规则；授权码模式要求 `redirect_uri` 域名预先登记，但未说明必须公网，也未承诺接受局域网地址；扫码 PKCE 不使用回调地址或 `AppSecret`，但官方将其适用场景描述为“无后端服务的第三方客户端”。因此此前默认的服务端授权码路线重新进入待决，不能凭空认定 LAN 回调会被平台接受。
- 影响文件：`plan.md`；暂不修改 `CONTEXT.md` 中既有 OAuth 模式边界，待授权路线确定后再同步术语。
- 验证：2026-08-01 通过官方实时页面核对上述四篇文档；文档只明确应用创建需填写“应用域名”，没有给出其对 PKCE 应用的校验规则或局域网例外。真实可接受值仍需在开放平台申请页面或审核沟通中确认。

## 2026-08-01 03:02 +0800
- 进度：确认愿意以个人开发者身份完成 115 开放平台实名认证，并申请仅供家庭媒体服务使用的自有应用；不引入企业开发者主体或第三方共享应用。接入路线继续采用有可保密 Go 服务端参与的 OAuth 授权码模式。
- 影响文件：`plan.md`；尚未修改应用代码、部署网络、配置或凭证存储。
- 验证：代码与部署文档核对确认当前家用部署机位于 NAT 后的 LAN，仅记录稳定 LAN IP/mDNS，未配置公网域名、HTTPS 入口或反向代理；下一步需确认是否已有可登记为 OAuth 回调的稳定域名。

## 2026-08-01 03:01 +0800
- 进度：确认当前尚无审核通过的 115 开放平台应用，因此没有可用于官方 OAuth 和真实 ED2K 云下载 POC 的自有 `AppID`/`AppSecret`。接口文档层面的可行性结论不变，但申请并审核开发者与应用是实施前置条件；不使用第三方应用凭证、不抓取或复用非官方令牌。
- 影响文件：`plan.md`；未修改 `CONTEXT.md`、代码、配置或 App 版本。
- 验证：仓库生产代码和 `.env.example` 均未发现 115 应用接入配置；官方“接入流程”说明开发者入驻和应用创建均需审核，平台标注各自在 7 个工作日内完成。

## 2026-08-01 03:00 +0800
- 进度：确认 115 替代 aMule 的首期完成边界采用“本地暂存”：115 云下载成功后，由家用部署机自动把对应文件拉取到受管理的本地暂存目录；不只停留在 115 云盘，也不自动创建视频库记录或触发转码、刮削。该边界与旧 aMule 工作台“下载落盘但不自动入库”的产品语义一致。
- 影响文件：`CONTEXT.md`、`plan.md`；尚未修改后端、管理端、数据库、部署配置或 App 版本。
- 验证：设计访谈进行中；已核对当前 `UploadService` 虽支持从本地路径入库，但首期明确不调用该能力。待授权前提、暂存目录与文件生命周期等决策收口后统一执行文档检查。

## 2026-08-01 02:56 +0800
- 进度：通过 `grill-with-docs` 开始核对以 115 开放平台替代已退役 aMule 的 ED2K 下载可行性。官方文档确认 `POST /open/offline/add_task_urls` 明确支持电驴链接，并可通过云下载任务列表取得完成资源 `file_id`，再经文件详情与下载地址接口形成“ED2K 链接 -> 115 云盘 -> 本地文件”的 API 路径；同时确认 115 所称“云下载完成”不等于家用部署机落盘或媒体入库，已先收紧该术语。当前仍在设计访谈中，尚未决定首期完成边界。
- 影响文件：`CONTEXT.md`、`plan.md`；未修改后端、管理端、数据库、部署配置或 App 版本，既有未跟踪 `docs/examples/` 保持不变。
- 验证：2026-08-01 已核对 115 官方“添加云下载链接任务”“获取用户云下载任务列表”“获取云下载配额信息”“获取文件列表”“获取文件(夹)详情”“获取文件下载地址”“接入流程”“开发须知”“授权码模式”“刷新 access_token”与“更新记录”页面；当前没有已审核应用凭证和可控账号调用证据，因此尚未执行真实 ED2K 任务、下载直链、断点续传或频控验证。
...
- 验证：`./gradlew --no-daemon :tv-app:testDebugUnitTest` 通过（631 项，0 失败、0 错误）；`./gradlew --no-daemon :tv-app:assembleDebug` 通过并生成 arm64-v8a、armeabi-v7a Debug APK；`git diff --check`、乱码扫描、旧播放器生产代码残留扫描、账号级轨道偏好调用核对及短视频/IPTV 变更扫描均通过。构建仅输出项目既有的 AGP 8.5.2 对 `compileSdk 35` 支持范围提示。

## 2026-07-31 21:04 +0800
- 进度：继续实施已确认的 TV 长视频 OTT 播放器重构。当前全量单测 642 项中 18 项失败，主要是旧测试仍锁定共享旧控制层、底部分集轨、外置轨道弹窗、续播卡和软切集；同时确认生产存储仍按视频 ID 保存轨道偏好，与已确认的账号级语义偏好不符。下一阶段收口 Media3 状态接线、300/500ms 加载反馈、临时错误两次自动重试、最终错误、生命周期暂停、亮屏、账号级轨道偏好、完成层和历史完成上报，再迁移冲突测试并清理无引用旧组件。
- 影响文件：`android-tv-app/tv-app/src/main` 的长视频播放器、偏好存储与登出边界，对应 `src/test`、TV 版本文件、`CONTEXT.md`、`plan.md`；继续排除短视频、短视频投放、IPTV、手机端与既有未跟踪 `docs/examples/`。
- 验证：`./gradlew --no-daemon :tv-app:testDebugUnitTest` 退出 1，642 项中 18 项失败；生产代码已编译，失败清单已记录为本轮测试迁移基线。待执行定向规则测试、TV 全量单测、`:tv-app:assembleDebug`、`git diff --check`、乱码与旧术语残留检查。

## 2026-07-31 20:42 +0800
- 进度：完成 TV 长视频统一会话第一阶段 RED。新增纯逻辑测试锁定真实能力显隐、面板/拖动/控制层/退出分层返回、播放中 4 秒隐藏、播放意图驱动亮屏、300/500ms 加载延迟与 15 秒超时、1/3 秒两次自动重试、控制焦点不环绕，以及 30 秒最小续播与 95% + 最多 5 分钟完成窗口。定向 Gradle 失败于新增会话类型和函数尚不存在，原生产代码编译与环境正常。
- 影响文件：`android-tv-app/tv-app/src/test/java/com/chee/videos/feature/tv/TvLongFormPlaybackSessionTest.kt`、`android-tv-app/tv-app/src/test/java/com/chee/videos/feature/tv/TvPlaybackHistoryPolicyTest.kt`、`plan.md`。
- 验证：`./gradlew --no-daemon :tv-app:testDebugUnitTest --tests com.chee.videos.feature.tv.TvLongFormPlaybackSessionTest --tests com.chee.videos.feature.tv.TvPlaybackHistoryPolicyTest` 按预期退出 1；失败均为目标 API unresolved reference，基线全量单测此前已通过。

## 2026-07-31 20:39 +0800
- 进度：完成 TV 长视频播放器重构实施前收口。新增 ADR `0018`，把 `0013` 的统一 ExoPlayer 内核升级为电影、`18+` 与电视剧共用的 OTT 播放会话、能力集和控制层，并正式取代 `0012` 的电视剧专属共享控制层。实施分五阶段：①纯状态模型与历史/返回/seek/错误测试；②Media3 生命周期、音频焦点、MediaSession 与轨道适配；③共享影院式控制层、右侧面板和完成层；④迁移单片与剧集宿主及单实例切集；⑤删除旧播放器、重复 overlay、双击返回与软切集状态并执行全量验证。
- 影响文件：`docs/adr/0012-tv-series-shared-controls-for-mixed-playback-engines.md`、`docs/adr/0018-tv-long-form-ott-playback-session.md`、`CONTEXT.md`、`plan.md`；实现预计涉及 `android-tv-app/tv-app/src/main` 与对应单测、`android-tv-app/tv-app/build.gradle.kts`，明确不修改短视频、短视频投放、IPTV、手机端或既有未跟踪 `docs/examples/`。
- 验证：正在执行 TV 全量单测基线；之后按阶段先跑纯逻辑与源文定向测试，再运行 `:tv-app:testDebugUnitTest`、`:tv-app:assembleDebug`、文档/中文编码/残留术语检查和 `git diff --check`，完成后按仓库规则使用中文提交。

## 2026-07-31 20:36 +0800
- 进度：确认 TV 长视频的全屏画面与操作安全区。视频始终铺到物理屏幕边缘，标题、时间轴、按钮、字幕、提示和面板内容限制在约 5% 的 TV 安全区；右侧面板背景可贴边但内容内缩。720p、1080p、4K 使用稳定的同一套 dp/sp 规格，不按分辨率放大字体，避免 overscan 裁切和布局漂移。至此播放器核心产品逻辑、遥控交互、生命周期与 UI 规则已完成访谈收口，下一阶段整理 ADR、实施计划并开始 TV App 重构。
- 影响文件：`CONTEXT.md`、`plan.md`；后续实现预计统一播放器布局 token、字幕/提示避让、安全区约束与多分辨率视觉验收，当前尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈已收口；待完成 ADR 与实施计划后执行文档差异、中文编码与术语一致性检查，再进入代码阶段。

## 2026-07-31 20:35 +0800
- 进度：确认底部控制区使用有边界的空间焦点导航，取消旧首尾循环。操作行左右移动到边界后停留，方向上进入时间轴，时间轴方向下回到播放/暂停；能力显隐或节点失效时也回退到播放/暂停。同步把旧 `controls 焦点环绕` 标记为已取代。
- 影响文件：`CONTEXT.md`、`plan.md`；后续实现预计重做主控制层 FocusRequester/focusProperties 接线、动态能力焦点回退与遥控导航测试，当前尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 20:34 +0800
- 进度：确认本轮 TV 长视频不提供画质选择入口。单一播放源不伪造清晰度选项，HLS 等自适应媒体由 Media3 自动选择码率并随网络调整；只有未来服务端明确提供多个真实可选版本时，才按能力集增加“自动 / 具体清晰度”二级面板。
- 影响文件：`CONTEXT.md`、`plan.md`；后续实现不增加画质 UI，仅需确保 Media3 默认自适应选择不被固定轨道覆盖，并在控制能力显隐测试中排除虚假入口，当前尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 20:32 +0800
- 进度：确认 TV 长视频不提供应用内音量或静音控件，不消费遥控器音量/静音键，也不保存独立音量；音量完全交给电视、功放或 HDMI-CEC，Media3 保持正常输出增益。音频焦点打断时暂停或等待短暂焦点恢复，不采用 ducking 后继续播放。
- 影响文件：`CONTEXT.md`、`plan.md`；后续实现预计锁定按键路由的音量透传与 Media3 音频属性/焦点行为，并补充遥控键测试，当前尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 20:31 +0800
- 进度：确认字幕和音轨采用即时选择模型。选项确认后立即生效并更新暖金勾选，面板保持打开且焦点留在当前行，不增加二次确认按钮；播放位置和播放/暂停状态保持不变，返回键关闭面板并恢复原入口焦点。切换失败时恢复上一有效选择，只在面板内显示轻量错误，不重启视频或进入最终播放错误态。
- 影响文件：`CONTEXT.md`、`plan.md`；后续实现预计统一轨道选择命令、乐观/失败回退状态、面板行选中表达与焦点测试，当前尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 20:30 +0800
- 进度：确认 TV 长视频字幕视觉与避让规则。优先遵循 Android TV 系统显式设置的字幕字号和样式；没有系统设置时采用最多两行、高对比白字配黑色描边或阴影的默认样式，并保持电视安全边距。本轮不增加字幕样式编辑器；主控制层或连播卡出现时字幕上移，右侧面板打开时字幕限制在未遮挡视频区域，禁止与操作 UI 重叠。
- 影响文件：`CONTEXT.md`、`plan.md`；后续实现预计接入系统字幕配置、统一 SubtitleView 样式与不同覆盖层下的安全区域，并补充源文/状态测试和实机视觉验收，当前尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 20:29 +0800
- 进度：确认 TV 长视频接入仅前台有效的 Media3 `MediaSession`。播放器页内的专用播放、暂停、快退、快进键及 HDMI-CEC 命令始终直接控制视频；面板打开时不关闭面板或改变焦点，快退/快进复用全局步长与连按合并并只显示瞬时反馈。系统仅获得片名、分集、进度和播放状态，离开播放器或进入后台即停用会话，不允许后台继续播放。
- 影响文件：`CONTEXT.md`、`plan.md`；后续实现预计增加 MediaSession 生命周期适配、统一命令路由、系统元数据与专用媒体键测试，当前尚未修改 Android 代码、依赖、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 20:27 +0800
- 进度：确认电视剧采用明确切集，不再沿用旧软切集。用户确认选集或下一集后立即保存旧集进度、停止旧音频、关闭面板并淡到黑色，在同一个 Media3 实例内切换媒体；超过 300ms 未起播才显示加载指示。手动选集按目标集历史续播，手动下一集与自动连播从头播放；新集失败时错误、重试和标题都归属于新目标，不回退旧集。同步把旧分集继续承接、双目标主语义、底部分集 rail 与返回旧集等冲突术语标记为已取代。
- 影响文件：`CONTEXT.md`、`plan.md`；后续实现预计重构剧集切换命令、历史保存、Media3 媒体项替换、加载/错误归属与相关测试，当前尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 20:23 +0800
- 进度：确认 TV 长视频有效续播区间。播放不足 30 秒不生成续播入口；自然结束或自动连播切集必定标记完成；手动退出只有同时满足进度达到 95% 且剩余不超过 5 分钟才视为完成，其余有效位置保存为续播历史。已完成内容在详情页只显示“播放”，不显示“继续播放”。
- 影响文件：`CONTEXT.md`、`plan.md`；后续实现预计统一历史快照、详情起播能力与自动连播完成上报的阈值，并补齐边界测试，当前尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 20:14 +0800
- 进度：确认 TV 长视频自然结束后的完成态。电影和 `18+` 结束后淡出到黑色完成层，显示片名与“返回详情 / 重新播放”；全剧最后一集显示“全剧已播完”与“返回详情 / 重播本集”。两类场景都不自动退出，默认焦点为“返回详情”，并把历史标记为已完成，避免详情页继续显示临近结尾的续播入口。结束态重播不扩展为主控制层常驻能力。
- 影响文件：`CONTEXT.md`、`plan.md`；后续实现预计增加统一完成态、历史完成上报、焦点与重播动作测试，并更新现有连播末集分支，当前尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 20:11 +0800
- 进度：确认 TV 长视频亮屏与屏保策略。正在播放，或用户仍期望继续播放但内核处于准备、缓冲状态时保持屏幕常亮；暂停、播放结束和最终错误时释放常亮，交回电视系统的屏保与待机设置。从屏保返回播放器后保持暂停，不自动出声。
- 影响文件：`CONTEXT.md`、`plan.md`；后续实现预计让播放器宿主的 keep-screen-on 状态由统一播放会话派生，并补齐状态切换与生命周期测试，当前尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 20:09 +0800
- 进度：确认 TV 长视频的前后台与音频焦点策略。播放器进入后台时立即保存进度并暂停，返回前台保持暂停；只有页面始终在前台且发生可恢复的短暂音频焦点中断时，才允许焦点恢复后自动继续。永久失焦、应用退后台，以及 HDMI、蓝牙等输出设备切换或断开后均保持暂停，等待用户显式继续。该规则将取代当前 `ON_RESUME` 按离开前状态自动播放的行为。
- 影响文件：`CONTEXT.md`、`plan.md`；后续实现预计调整统一播放会话的生命周期事件、Media3 音频焦点配置、输出设备变化处理及相关测试，当前尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 19:53 +0800
- 进度：确认单片与剧集收口为统一长视频播放会话，集中管理播放、控制模式、seek、轨道、缓冲、错误、历史和生命周期。电影/`18+` 使用基础能力，剧集额外声明选集、下一集和自动连播；Media3 只通过适配边界接入，同页切集复用单一播放器实例。现有两个播放器页面将缩为薄路由宿主，迁移完成后删除旧 `LongFormVideoPlayer`、重复 overlay 和分散状态。
- 影响文件：`CONTEXT.md`、`plan.md`；后续实现预计重构 TV 长视频播放会话、Media3 适配、主控制层、单片/剧集宿主与相关测试，当前尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 19:52 +0800
- 进度：确认播放器焦点视觉作为全局“只缩放”规则的局部例外。图标按钮聚焦使用暖金圆底、深色图标和轻微放大；右侧面板行使用深色提亮与暖金描边；当前选中项另用暖金勾选标记，区分焦点与选择。动画约 200ms、无光晕并服从系统关闭动画设置。同步标记旧双层 glow 已取代。
- 影响文件：`CONTEXT.md`、`plan.md`；尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 19:45 +0800
- 进度：确认本轮 TV 长视频固定 `1.0x` 播放，不加入倍速入口或倍速偏好。倍速若以后出现明确需求，作为独立播放设置能力设计，不占用当前主控制层或右侧二级面板。同步标记旧“播放器只换皮且含倍速选择”的阶段性约定已被整体重构取代。
- 影响文件：`CONTEXT.md`、`plan.md`；尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 19:44 +0800
- 进度：确认 TV 长视频固定保持原始宽高比并完整显示，允许黑边，不提供裁切铺满、拉伸或画面比例切换。该规则统一覆盖电影、`18+`、电视剧分集和 Dolby Vision/HDR 内容，播放器 UI 不新增画面模式入口。
- 影响文件：`CONTEXT.md`、`plan.md`；尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 19:38 +0800
- 进度：确认播放器操作按真实能力显隐：无字幕隐藏字幕，只有一条音轨隐藏音轨，单片隐藏选集与下一集，剧集无下一可播放分集时隐藏下一集但保留选集。隐藏后操作行以稳定间距重排，播放/暂停仍为默认焦点；不保留禁用按钮，也不让用户进入空面板。同步取代旧 Dolby Vision 分支“无轨道仍显示入口”的约定。
- 影响文件：`CONTEXT.md`、`plan.md`；尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 19:37 +0800
- 进度：确认字幕和音轨采用当前登录用户级语义偏好。音轨保存语言与类型；字幕保存“关闭 / 自动 / 指定语言”；跨影片和分集自动匹配，不保存底层轨道 ID，也不跨账号泄漏。当前内容缺少匹配轨道时临时回退媒体默认，但不得覆盖用户原偏好。同步将旧按 `videoId` 保存的粒度标记为已取代。
- 影响文件：`CONTEXT.md`、`plan.md`；尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 18:52 +0800
- 进度：确认 TV 长视频采用“自动恢复优先、人工操作兜底”。临时网络或媒体读取错误在保留位置和最后画面的前提下自动重试两次，约 1 秒、3 秒退避；格式不支持、解码失败和 Dolby Vision 安全阻断直接失败。最终居中错误层只显示“重试”“返回详情”和有诊断时的次级入口；手动重试沿用失败位置并恢复播放，不切换内核或播放源。该统一语义取代单片/剧集各自的软重试 UI 分支。
- 影响文件：`CONTEXT.md`、`plan.md`；尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 18:50 +0800
- 进度：确认 TV 长视频统一加载/缓冲反馈。首次起播用纯黑画布，超过 300ms 才显示无文案居中加载指示；首帧后的缓冲保留最后画面，超过 500ms 才显示较小加载指示；缓冲期间不自动隐藏当前控制层。连续启动或缓冲超过 15 秒统一升级为可恢复错误，不允许无限转圈。
- 影响文件：`CONTEXT.md`、`plan.md`；尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 18:46 +0800
- 进度：确认电视剧采用成熟 OTT 片尾连播。存在下一集时在片尾 10 秒显示右下角卡片，包含下一集标题、倒计时、“立即播放”和“观看片尾”，默认焦点为“立即播放”；暂停、精确拖动或右侧面板打开时冻结倒计时。“观看片尾”只取消本集自动跳转，不改全局开关；没有下一集时不倒计时，结束后直接显示简洁的“全剧已播完”。
- 影响文件：`CONTEXT.md`、`plan.md`；尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 18:44 +0800
- 进度：确认“从头播放”属于详情页起播意图，不进入播放器常驻控制。电影、`18+` 和电视剧目标有有效历史时显示“继续播放”与“从头播放”，无历史时只显示“播放”；进入播放器后仍可通过精确时间轴回到开头。该决定只扩展详情页播放按钮区，不重构详情页主体。
- 影响文件：`CONTEXT.md`、`plan.md`；后续实现范围将包含 TV 长视频与电视剧详情页的起播操作，但当前尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 18:41 +0800
- 进度：确认 TV 长视频统一自动续播。进入有有效历史进度的单片或分集时直接从历史位置开始，仅在左下角短暂显示无焦点、无按钮的“已从 HH:MM:SS 继续播放”提示；不暂停、不二次确认。同一目标在一次会话只提示一次。同步删除旧续播双按钮卡、倒计时、永久 dismiss 与内嵌槽位等状态语义。
- 影响文件：`CONTEXT.md`、`plan.md`；尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 18:39 +0800
- 进度：确认取消 TV 长视频播放器“双击返回退出”，改为单次分层返回：右侧面板先关闭，精确拖动先取消，主控制层先收起，纯观看状态下一次返回保存进度并立即回详情。同步清理领域词汇中 Dolby Vision、软重试、焦点、连播和续播对旧退出确认提示的依赖；TV 短视频和 TV 首页根退出仍属本轮排除范围，保持各自既有语义。
- 影响文件：`CONTEXT.md`、`plan.md`；尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 18:36 +0800
- 进度：确认字幕、音轨和选集统一使用右侧滑入深色实体面板。字幕/音轨为单列选项；选集顶部切换季，主体以可滚动剧集行展示集号、标题和观看进度。返回键先关闭面板并恢复原入口焦点。同步删除与新设计冲突的底部横向选集轨、三层上下焦点栈、四项 controls 和只读进度条等旧术语，保留不可播分集弱化、当前集定位和双态识别等仍成立的产品语义。
- 影响文件：`CONTEXT.md`、`plan.md`；尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 18:32 +0800
- 进度：确认 TV 长视频主控制层采用“顶部轻信息 + 底部单层控制带”。左上角只显示片名，剧集显示剧名、集号和集标题；底部为完整时间轴与一行图标操作，左侧放后退、播放/暂停、前进及剧集下一集，右侧放字幕、音轨及剧集选集，默认焦点落播放/暂停。画面中心只保留瞬时播放、暂停和跳转反馈，不设置常驻控制岛。
- 影响文件：`CONTEXT.md`、`plan.md`；尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 18:29 +0800
- 进度：确认 TV 长视频采用影院式视觉：全屏视频上只叠顶部/底部黑色渐隐遮罩，主信息使用高对比白色，暖金仅表达进度、焦点和当前选中；核心控制层不套玻璃卡片，字幕、音轨、选集等二级能力改用克制的深色实体面板。同步将旧“播放器第三批暗玻璃”与“夜台玻璃面板”标记为已被本轮长视频播放器视觉取代，连接、配对及共享状态页不受影响。
- 影响文件：`CONTEXT.md`、`plan.md`；尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 18:25 +0800
- 进度：确认 TV 长视频控制层自动隐藏规则。正常播放且连续 4 秒无有效操作时隐藏，任一有效遥控操作重置计时；暂停时保持可见并允许返回键主动收起；精确拖动、字幕、音轨、选集、续播、连播和错误等持续交互状态均禁止自动隐藏，避免计时器抢走焦点或中断决策。
- 影响文件：`CONTEXT.md`、`plan.md`；尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 18:09 +0800
- 进度：确认 TV 长视频播放器采用双层 seek。控制层隐藏时左右键继续按全局步长快速跳转，连续输入累计目标并约 300ms 合并提交；进度条获得焦点后进入精确拖动，只移动待确认目标，确认键提交、返回键取消。首期不依赖服务端缩略图，只显示目标时间和进度，缩略图预览保留为后续增强。
- 影响文件：`CONTEXT.md`、`plan.md`；尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 17:35 +0800
- 进度：确认成熟播放器逻辑与 UI 的参考优先级高于当前 TV 播放器旧约定；仓库 `ui-ux-pro-max` 检索只作为 OLED 深色画布、最少 chrome、高对比焦点、减少 UI 业务状态等通用护栏，具体大屏行为继续按成熟 OTT 播放器的共同模式收口。确认遥控器主操作：控制层隐藏时确认键直接播放/暂停并唤起控制层，方向下键只唤起控制层、不改变播放状态。
- 影响文件：`CONTEXT.md`、`plan.md`；尚未修改 Android 代码、资源或版本号。
- 验证：设计访谈进行中，待全部决策收口后统一执行文档差异、中文编码与术语一致性检查。

## 2026-07-31 17:31 +0800
- 进度：确认 TV 长视频播放器采用“沉浸式 OTT + 按需高级能力”体验基线：默认画面优先，主动唤起后只展示标题、进度与少量核心操作，字幕、音轨、选集进入二级界面，诊断仅在故障上下文出现。同步处理 `CONTEXT.md` 冲突，将“保持既有控制层语义”和“只先抽电视剧控制层”标记为迁移期约定并由本轮整体重构取代。
- 影响文件：`CONTEXT.md`、`plan.md`；尚未修改 Android 代码、资源或版本号。
- 验证：`git diff --check` 待本轮访谈决策更新后统一复验；当前不执行 App 构建。

## 2026-07-31 17:27 +0800
- 进度：通过 `grill-with-docs` 开始收敛 TV 长视频播放器重构。已确认本次不是单纯替换控制条，而是同时重构播放器结构与 UI；范围统一覆盖电影、`18+` 和电视剧分集的本地全屏播放，明确排除短视频、短视频投放、IPTV 与手机端播放器。既有播放器交互约定不再自动作为约束，后续按成熟 TV 播放器体验逐项重新决策。
- 影响文件：本轮设计访谈当前仅更新 `CONTEXT.md` 与 `plan.md`；尚未修改 Android 代码、资源或版本号，既有未跟踪 `docs/examples/` 不纳入。
- 验证：待设计树全部收口后执行 Markdown 差异、中文编码与术语一致性检查；当前不执行 App 构建。
...
- 验证：`./gradlew --no-daemon :tv-app:testDebugUnitTest :tv-app:assembleDebug` 全绿；修复后交回子代理复审。

## 2026-07-26 18:53 +0800
- 进度：完成 TV 整体优化批C（视觉打磨）。启动器身份三件套落地：暖金字形自适应图标 + TV banner + `Theme.VideoHome` 深色 `windowBackground` 消白闪（splash 三件套不动）。抽共享 `core/ui/TvActionButton`（Primary 金底/Secondary 暗玻璃、canonical 18/12 内边距、缩放 1.06/1.05），收敛首页 hero、长视频详情、海报墙排序三处本地按钮，连接/配对页薄封装委托，`TvSeriesDetailScreen` 按授权豁免；四个既有审计测试完成目标迁移（只收紧），新增 `TvActionButtonSpecTest`、`TvLauncherIdentitySpecTest`。IPTV 收口：选中行金底白字对比度 2.2:1 修复为 Canvas 前景 9.5:1、冷色面板改 `CanvasRaised`、非对称 18dp 圆角回归 8dp、台标白纱底改暗玻璃、分组名补省略。投放页两处裸白字状态改共享 `TvPageLoadingState`/`TvErrorState`；外壳退出提示冷灰改 `SurfaceMuted`；配对页错误行加警示图标与省略保护。方案中的 `TvPosterImage` 共享图片组件与 `TvFeaturedPoster` 去重经核实放弃（前者收益不成立且 shared-element 风险高，后者是 hero spec 测试切片锚点），连接页行内 loading 保持既有决策形态。TV 版本 139→140。
- 影响文件：`AndroidManifest.xml`、`res/values/themes.xml`、`res/values/colors.xml`（新增）、`res/drawable/ic_launcher_glyph.xml`、`ic_launcher_background.xml`、`tv_banner.xml`（新增）、`res/mipmap-anydpi-v26/ic_launcher.xml`、`ic_launcher_round.xml`（新增）、`core/ui/TvActionButton.kt`（新增）、`feature/tv/TvCatalogScreen.kt`、`TvLongFormDetailScreen.kt`、`TvPosterWallScreen.kt`、`TvIptvScreen.kt`、`TvRemotePlaybackScreen.kt`、`tv/TvShellApp.kt`、`tv/TvPairingScreen.kt`、`feature/connection/ConnectionScreen.kt`、`tv-app/build.gradle.kts`、测试迁移 4 个 + 新增 2 个、`CONTEXT.md`（新增「TV 端整体优化：视觉收口」小节）、`plan.md`。未跟踪 `.superpowers/` 保持原样且不读取、不修改、不纳入提交。
- 验证：`./gradlew --no-daemon :tv-app:testDebugUnitTest` 全绿；`:tv-app:assembleDebug` 通过（AAPT 编译校验新图标/banner 资源）。图标与 banner 在真实 leanback 启动器上的观感、IPTV 选中行新配色待真机安装验收；接下来按方案执行子代理独立评审。

## 2026-07-26 18:33 +0800
- 进度：完成 TV 整体优化批B（流畅度）。三处随帧动画做 defer-read（`tvFocusableScaleOnly` 焦点缩放——遥控器连按方向键时失焦/得焦双节点弹簧期逐帧重组的主要掉帧源、`tvStaggerEntry` 海报墙入场、`TvFeaturedHero` 120s Ken Burns——首页常驻逐帧重组），动画 State 保留句柄并延后到 `graphicsLayer` 读值，视觉输出不变，`TvFeaturedHeroMotionSpecTest` 零改动通过。`AppChrome.PageGradient/HeroGradient` get() 属性改 val 单例。`TvCatalogScreen` 三处解析函数 `remember` 记忆化。Lazy key 硬化五处（重名 section/空 id 演员两处为真实崩溃隐患）并给目录外层 LazyColumn 与海报墙补 `contentType`。`VideoApp` 实现 Coil `ImageLoaderFactory`（内存 25%、磁盘 128MB、全局 crossfade(false)、respectCacheHeaders(false)；allowRgb565 留待实机）。`TvLongFormMedia3Player` 4Hz 快照轮询发射端结构相等去重。`TvSeriesDetailScreen` 演员头像 `SubcomposeAsyncImage` 因 `TvSeriesDetailActionSpecTest` 锁定按方案约定不动。TV 版本 138→139。
- 影响文件：`core/ui/AppChrome.kt`、`core/ui/TvFocus.kt`、`core/ui/TvListMotion.kt`、`feature/tv/TvCatalogScreen.kt`、`feature/tv/TvSeriesDetailScreen.kt`、`feature/tv/TvLongFormDetailScreen.kt`、`feature/tv/TvPosterWallScreen.kt`、`feature/tv/TvIptvScreen.kt`、`feature/tv/TvLongFormMedia3Player.kt`、`VideoApp.kt`、`tv-app/build.gradle.kts`、新增测试 `core/ui/TvDeferredAnimationReadSpecTest.kt`、`feature/tv/TvSmoothnessAuditSpecTest.kt`、`TvImageLoaderFactorySpecTest.kt`、`CONTEXT.md`（新增「TV 端整体优化：流畅度基线」小节）、`plan.md`。未跟踪 `.superpowers/` 保持原样且不读取、不修改、不纳入提交。
- 验证：`./gradlew --no-daemon :tv-app:testDebugUnitTest` 全绿；`:tv-app:assembleDebug` 通过。遥控器连按焦点移动、首页空闲 CPU 占用、货架划回海报即时命中等观感差异待真机安装验收；批C（视觉）按 18:10 方案条目继续。

## 2026-07-26 18:26 +0800
- 进度：完成 TV 整体优化批A（内存泄漏与生命周期）。`TvIptvScreen` 事件回调加 `released` 守卫并在退出时 `removeCallbacksAndMessages(null)`，收口「守卫→解监听→清队列→release」顺序，迟到的 LibVLC 事件不再触碰已释放播放器；保留视图解绑独立 effect 的既有决策（`TvIptvPlayerViewLayoutTest` 锁定），仅补注释说明销毁次序依据。`TvRemoteCoordinatorViewModel` 的 dismiss 上报由无限 1Hz 重试改为单轮 5 次指数退避（1s→16s 封顶）+ 任务表清理 + `dismissedSessionIds` 容量 16 按插入序淘汰，宏观仍由 5s 会话轮询重触发保持「上报直到成功」。`TvPairingScreen` 配对轮询接入前台门控（`pollingAllowed.first { it }` + ON_START/ON_STOP 观察者）。计划中的 LeakCanary 因本机网络对 Gradle JVM 的 TLS 握手中断（curl 可通）暂缓接入，已记入 `CONTEXT.md`。TV 版本 `versionCode` 137→138、`versionName` 0.1.137→0.1.138。
- 影响文件：`android-tv-app/tv-app/src/main/java/com/chee/videos/feature/tv/TvIptvScreen.kt`、`tv/TvRemoteCoordinatorViewModel.kt`、`tv/TvPairingScreen.kt`、`android-tv-app/tv-app/build.gradle.kts`、`src/test/.../tv/TvRemoteCoordinatorLifecycleSpecTest.kt`（断言迁移为有界重试形态）、`src/test/.../feature/tv/TvLifecycleHygieneSpecTest.kt`（新增）、`CONTEXT.md`（新增「TV 端整体优化：生命周期与泄漏边界」小节）、`plan.md`。未跟踪 `.superpowers/` 保持原样且不读取、不修改、不纳入提交。
- 验证：`./gradlew --no-daemon :tv-app:testDebugUnitTest` 全绿；`:tv-app:assembleDebug` 通过。真机后台挂起/恢复下的配对轮询与 IPTV 退出稳定性待安装验收；批B（流畅度）、批C（视觉）按 18:10 方案条目继续。

## 2026-07-26 18:10 +0800
- 进度：完成「TV App 整体优化（样式优雅好看、流畅度高、无内存泄漏）」总体方案整理。已完成全量代码摸底与 `CONTEXT.md` 既有决策核对，并由多个子代理并行产出流畅度与视觉两份细化方案后合并（内存泄漏方案基于摸底报告直接收口）。范围决策：不升级 Compose/Kotlin 工具链（BOM 2024.10.01 锁定，`TvMainActivity` 主循环 hover 守卫的根治留待后续升级专项）；不做需真机基准的 Baseline Profile；编译图外的手机端参考源码按 `AGENTS.md` 约定保留不删；视觉严格在既有「暖金 `#E8B85B` + 暗玻璃 + 焦点只缩放」基线内打磨，不重设计、不动 `CONTEXT.md` 已授权的例外区（`TvSeriesDetailScreen` 参考图还原区、海报墙 1.08 缩放+黑落影、侧边菜单静态高亮、Ken Burns 参数、`TvDetailPanel` 28dp 圆角）。方案分三批独立提交：
  - 批A 内存泄漏与生命周期（`versionCode` 137→138）：`TvIptvScreen` 主线程 Handler 在 dispose 时 `removeCallbacksAndMessages(null)` 并加已释放守卫、`attachViews/detachViews/setEventListener/release` 合并为单一 `DisposableEffect` 收口顺序；`TvRemoteCoordinatorViewModel` 的 dismiss 会话上报改为有界重试（上限+退避，失败达上限后移除 `dismissJobs` 条目）、`dismissedSessionIds` 集合加容量上界；`TvPairingScreen` 的 `while(true)` 配对轮询接入生命周期门控（对齐 `TvShellApp` 对远程协调器的 ON_START/ON_STOP 门控方式）；`tv-app` 增加 `debugImplementation` LeakCanary 2.14 作为后续泄漏回归探测；新增源码审计型 spec 测试锁 Handler 清理与轮询门控形态。
  - 批B 流畅度（138→139）：`AppChrome.PageGradient/HeroGradient` 由 `get()` 属性改为 `val` 单例；`tvFocusableScaleOnly`、`tvStaggerEntry`、`TvFeaturedHero` Ken Burns 三处将动画 State 读取延后到 `graphicsLayer` lambda 内（defer-read，把逐帧重组降级为仅重绘；Ken Burns 保持 `TvFeaturedHeroMotionSpecTest` 全部 token 断言零改动通过）；`TvCatalogScreen` 的 `resolveTvFeaturedContent`/`resolveTvCatalogInitialFocusTarget`/`buildTvTypedHomeSections(...).last().title` 用 `remember` 记忆化；LazyList key 硬化五处（目录 section 键加 index 兜底、目录 more 卡改局部常量键、剧集详情演员键 `itemsIndexed` 加 index、长视频详情演员行补 key、IPTV 频道键加分组前缀）并只在真正混排的目录外层 `LazyColumn` 与海报墙网格补 `contentType`；`VideoApp` 实现 `ImageLoaderFactory`（内存缓存 25%、磁盘 128MB、全局 `crossfade(false)`、`respectCacheHeaders(false)`；`allowRgb565` 因 IPTV 台标渐变风险留待实机确认不上）；`TvLongFormMedia3Player` 4Hz 快照轮询加发射端去重（暂停/缓冲态不再下发相同快照）；`TvSeriesDetailScreen` 演员头像 `SubcomposeAsyncImage` 是否改 `AsyncImage` 视既有 spec 断言而定（若被锁则不动）。新增 spec 测试：hero defer-read 形态、`AppChrome` 渐变单例、`ImageLoaderFactory` 存在性、LazyList key 负面审计。
  - 批C 视觉打磨（139→140）：启动器身份三件套——手写 vector 自适应图标（暖金屏框+播放三角字形、暗玻璃渐变底）、320×180dp TV banner（layer-list，暖金侧光+字形+8dp 压边）、`Theme.VideoHome` 补 `android:windowBackground=#FF040508` 消启动白闪（新增 `colors.xml` 镜像 `AppChrome.Canvas`，不动 splash 三件套）；抽共享 `core/ui/TvActionButton.kt`（Primary 金底/Secondary 暗玻璃，canonical padding 18/12、titleSmall SemiBold、缩放 1.06/1.05），收敛目录 hero、长视频详情、海报墙排序、连接页、配对页 7 处重复实现（`TvSeriesDetailScreen` 按 `CONTEXT.md` 授权豁免；`ConnectionScreen`/`TvPairingScreen` 保留薄封装以保住既有金底前景断言）；抽共享 `core/ui/TvPosterImage.kt`（暗玻璃占位、错误态图标兜底、`TvMotionTokens.ImageCrossfadeMs=220` 逐请求 crossfade；海报墙 shared-element modifier 必须原样透传给内部 `AsyncImage`，播放器等首帧封面三处与 IPTV 台标状态机不接入）；状态屏对齐（`TvRemotePlaybackScreen` 两处裸 `Text` 换 `TvPageLoadingState`/`TvErrorState`、`ConnectionScreen` 行内加载换 `TvInlineLoadingState`、`TvShortFeedScreen` 状态面板改调共享件并给共享件加 `maxWidth` 参数、配对页错误行加警示图标与 `maxLines`）；IPTV 硬编码色收口（选中频道行白字改 `AppChrome.Canvas` 修复 2.16:1→9.46:1 对比度缺陷、冷色黑名单色换 `AppChrome` token、非对称圆角 18dp 回归 `RadiusDp` 8dp、频道/分组名补 `maxLines`）；`TvShellApp`/`TvRemotePlaybackScreen` 残留旧冷灰与裸白替换为 `AppChrome` token；相关既有 spec 测试做审计目标迁移（只收紧不放松），新增启动器身份、共享按钮、图片加载三个 spec 测试并把 `TvShapeAuditTest` 正则扩展到非对称圆角写法。
- 影响文件：批A `TvIptvScreen.kt`、`TvRemoteCoordinatorViewModel.kt`、`TvPairingScreen.kt`、`tv-app/build.gradle.kts`；批B `AppChrome.kt`、`TvFocus.kt`、`TvListMotion.kt`、`TvCatalogScreen.kt`、`TvSeriesDetailScreen.kt`、`TvLongFormDetailScreen.kt`、`TvIptvScreen.kt`、`TvLongFormMedia3Player.kt`、`VideoApp.kt`；批C `AndroidManifest.xml`、`res/`（新增 mipmap/drawable/colors）、`TvActionButton.kt`（新增）、`TvPosterImage.kt`（新增）、`TvMotion.kt`、`TvStateFeedback.kt`、`TvRemotePlaybackScreen.kt`、`ConnectionScreen.kt`、`TvShortFeedScreen.kt`、`TvShellApp.kt`；各批含对应新增/迁移 spec 测试与 `plan.md`、`CONTEXT.md`。未跟踪 `.superpowers/` 保持原样且不读取、不修改、不纳入提交。
- 验证：每批完成后运行 `./gradlew :tv-app:testDebugUnitTest` 与 `:tv-app:assembleDebug`，逐批 bump `versionCode`/`versionName` 并独立提交（138/139/140）；三批完成后派子代理独立评审、修复问题并复审至无阻塞问题；crossfade 手感、图标/banner 在真实启动器上的观感、IPTV 选中行配色与 RGB565 决策留待真机安装验收。
...
- 验证：代码只读核对 `TvLongFormMedia3Player.kt`、`TvRemotePlaybackScreen.kt` 及现有投屏页测试；待确认诊断事件集合后再实施代码。

## 2026-07-26 11:03 +0800
- 进度：完成 TV 短视频播放诊断来源日志脱敏边界沉淀。下一项待确认单次播放尝试的关键日志事件；当前未修改播放器、接口、转码产物或版本号。
...
## 2026-07-26 11:02 +0800
- 进度：用户确认 TV 短视频播放诊断的来源日志脱敏边界：只记录视频 ID、可比较的播放路径与档位，不记录服务器主机地址或 `Authorization`。当前继续收口单次播放尝试的事件范围，尚未实施代码。
- 影响文件：`CONTEXT.md`、`plan.md`；未跟踪 `.superpowers/` 保持原样且不读取、不修改、不纳入提交。
- 验证：待执行 `git diff --check -- CONTEXT.md plan.md`、本次 Markdown Unicode 替换字符扫描、精确暂存与提交范围检查；文档变更无需构建或测试。

## 2026-07-26 11:01 +0800
- 进度：完成“TV 短视频播放诊断”术语沉淀，诊断限于内部日志、按播放尝试关联且不改变播放决策。下一项待确认来源日志的脱敏边界，当前未修改播放器、接口、转码产物或版本号。
- 影响文件：`CONTEXT.md`、`plan.md`；未跟踪 `.superpowers/` 保持原样且不读取、不修改、不纳入提交。
- 验证：`git diff --check -- CONTEXT.md plan.md` 通过；两份 Markdown Unicode 替换字符扫描无匹配；文档变更无需构建或测试。

## 2026-07-26 11:00 +0800
- 进度：用户确认首轮 TV 短视频花屏修复加入内部 Media3 播放诊断，按播放尝试记录播放目标、实际来源、解码器、输入视频格式与错误结果；不新增用户可见诊断入口或服务端接口，诊断不参与播放决策。尚未实施代码。
- 影响文件：`CONTEXT.md`、`plan.md`；未跟踪 `.superpowers/` 保持原样且不读取、不修改、不纳入提交。
- 验证：待执行 `git diff --check -- CONTEXT.md plan.md`、本次 Markdown Unicode 替换字符扫描、精确暂存与提交范围检查；文档变更无需构建或测试。

## 2026-07-26 10:59 +0800
- 进度：完成 TV 短视频花屏首轮验收标准沉淀。下一项待确认是否在首次修复中增加仅供现场排查的播放诊断信息，当前未修改播放器、接口、转码产物或版本号。
- 影响文件：`CONTEXT.md`、`plan.md`；未跟踪 `.superpowers/` 保持原样且不读取、不修改、不纳入提交。
- 验证：`git diff --check -- CONTEXT.md plan.md` 通过；两份 Markdown Unicode 替换字符扫描无匹配；文档变更无需构建或测试。

## 2026-07-26 10:58 +0800
- 进度：用户确认 TV 短视频花屏首轮验收：本地与投屏的实际播放地址必须完全一致，且无自动回退或重转码；同源后仍花屏则进入媒体产物或设备硬解兼容性排查，不归因于投屏控制层。尚未实施代码。
- 影响文件：`CONTEXT.md`、`plan.md`；未跟踪 `.superpowers/` 保持原样且不读取、不修改、不纳入提交。
- 验证：待执行 `git diff --check -- CONTEXT.md plan.md`、本次 Markdown Unicode 替换字符扫描、精确暂存与提交范围检查；文档变更无需构建或测试。

## 2026-07-26 10:53 +0800
- 进度：完成 TV 短视频花屏首轮修复范围沉淀：只让投屏入口与本地短视频入口同源，不引入自动 `compat` 回退或重转码副作用。继续讨论现场诊断信息是否随首轮修复加入。
- 影响文件：`CONTEXT.md`、`plan.md`；未跟踪 `.superpowers/` 保持原样且不读取、不修改、不纳入提交。
- 验证：`git diff --check -- CONTEXT.md plan.md` 通过；两份 Markdown Unicode 替换字符扫描无匹配；文档变更无需构建或测试。

## 2026-07-26 10:52 +0800
- 进度：用户确认 TV 短视频花屏首轮修复只恢复投屏与本地入口的同源播放，不添加自动 `compat` 回退，也不在本次触发重转码。当前继续收口现场诊断范围，尚未实施代码。
- 影响文件：`CONTEXT.md`、`plan.md`；未跟踪 `.superpowers/` 保持原样且不读取、不修改、不纳入提交。
- 验证：待执行 `git diff --check -- CONTEXT.md plan.md`、本次 Markdown Unicode 替换字符扫描、精确暂存与提交范围检查；文档变更无需构建或测试。
...
- 验证：`git diff --check -- CONTEXT.md plan.md` 通过；两份文档的 Unicode 替换字符扫描无匹配；差异检查确认无代码、接口或版本号改动，文档变更无需构建或测试。

## 2026-07-25 14:20 +0800
- 进度：用户确认短视频合集目录要求登录，但所有已登录用户看到相同的可见合集，不按角色、创建者或个人状态过滤；私有合集、订阅和个性化排序不纳入首期。下一步进行设计收口检查，本轮不实施代码。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；不修改手机端、后端接口、版本号或现有播放行为。
- 验证：待执行 `git diff --check`、中文替换字符扫描、提交范围检查与文档提交；文档变更无需构建或测试。

## 2026-07-25 14:15 +0800
- 进度：完成短视频合集过期入口术语沉淀；首期权限范围、指标埋点和实现拆分仍待逐项确认。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；未跟踪 `.superpowers/` 保持原样且不纳入提交。
- 验证：`git diff --check -- CONTEXT.md plan.md` 通过；两份文档的 Unicode 替换字符扫描无匹配；差异检查确认无代码、接口或版本号改动，文档变更无需构建或测试。

## 2026-07-25 14:14 +0800
- 进度：用户确认缓存目录中的合集被后台停用或清空时，内容页显示“该合集已下线或暂无可播放内容”并提供返回；这次返回目录时自动刷新一次移除过期卡片。继续讨论首期权限范围，本轮不实施代码。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；不修改手机端、后端接口、版本号或现有播放行为。
- 验证：待执行 `git diff --check`、中文替换字符扫描、提交范围检查与文档提交；文档变更无需构建或测试。

## 2026-07-25 14:12 +0800
- 进度：完成短视频合集目录缓存术语沉淀；后台下线合集时的内容页反馈和首期权限边界仍待逐项确认。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；未跟踪 `.superpowers/` 保持原样且不纳入提交。
- 验证：`git diff --check -- CONTEXT.md plan.md` 通过；两份文档的 Unicode 替换字符扫描无匹配；差异检查确认无代码、接口或版本号改动，文档变更无需构建或测试。

## 2026-07-25 14:11 +0800
- 进度：用户确认短视频合集目录只在当前应用会话与返回导航中保留数据、已加载页和滚动位置，不写入磁盘，应用重启后重新请求；继续讨论后台下线合集时的内容页反馈，本轮不实施代码。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；不修改手机端、后端接口、版本号或现有播放行为。
- 验证：待执行 `git diff --check`、中文替换字符扫描、提交范围检查与文档提交；文档变更无需构建或测试。

## 2026-07-25 14:07 +0800
- 进度：完成短视频合集目录分页术语沉淀；首期缓存范围与返回栈恢复边界仍待逐项确认。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；未跟踪 `.superpowers/` 保持原样且不纳入提交。
- 验证：`git diff --check -- CONTEXT.md plan.md` 通过；两份文档的 Unicode 替换字符扫描无匹配；差异检查确认无代码、接口或版本号改动，文档变更无需构建或测试。

## 2026-07-25 14:06 +0800
- 进度：用户确认短视频合集目录每页 20 个合集，滚动接近底部自动加载下一页，不展示页码或“加载更多”按钮；与现有图集目录分页口径保持一致。继续讨论首期缓存范围，本轮不实施代码。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；不修改手机端、后端接口、版本号或现有播放行为。
- 验证：待执行 `git diff --check`、中文替换字符扫描、提交范围检查与文档提交；文档变更无需构建或测试。

## 2026-07-25 14:00 +0800
- 进度：完成短视频合集目录接口契约沉淀；目录分页规模与缓存范围仍待逐项确认。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；未跟踪 `.superpowers/` 保持原样且不纳入提交。
- 验证：`git diff --check -- CONTEXT.md plan.md` 通过；两份文档的 Unicode 替换字符扫描无匹配；差异检查确认无代码、接口或版本号改动，文档变更无需构建或测试。

## 2026-07-25 13:59 +0800
- 进度：用户确认首期新增鉴权短视频合集目录接口 `GET /api/v1/short/collections?page&page_size`，返回目录摘要；合集内容页继续复用按 `collection_id` 发现短视频的既有接口，不新增合集详情接口。继续讨论目录分页规模与缓存范围，本轮不实施代码。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；不修改手机端、后端接口、版本号或现有播放行为。
- 验证：待执行 `git diff --check`、中文替换字符扫描、提交范围检查与文档提交；文档变更无需构建或测试。

## 2026-07-25 13:50 +0800
- 进度：完成短视频合集目录错误反馈术语沉淀；后端公开接口范围和分页参数仍待逐项确认。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；未跟踪 `.superpowers/` 保持原样且不纳入提交。
- 验证：`git diff --check -- CONTEXT.md plan.md` 通过；两份文档的 Unicode 替换字符扫描无匹配；差异检查确认无代码、接口或版本号改动，文档变更无需构建或测试。

## 2026-07-25 13:49 +0800
- 进度：用户确认短视频合集目录在刷新失败时保留缓存列表并提示可重试，首次无缓存加载失败时显示完整错误态与重试入口，不将失败伪装为空目录；继续讨论后端接口边界，本轮不实施代码。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；不修改手机端、后端接口、版本号或现有播放行为。
- 验证：待执行 `git diff --check`、中文替换字符扫描、提交范围检查与文档提交；文档变更无需构建或测试。

## 2026-07-25 13:47 +0800
- 进度：完成短视频合集目录刷新术语沉淀；加载/错误反馈和初期接口范围仍待逐项确认。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；未跟踪 `.superpowers/` 保持原样且不纳入提交。
- 验证：`git diff --check -- CONTEXT.md plan.md` 通过；两份文档的 Unicode 替换字符扫描无匹配；差异检查确认无代码、接口或版本号改动，文档变更无需构建或测试。

## 2026-07-25 13:46 +0800
- 进度：用户确认短视频合集目录首次进入自动加载，后续不因首页内容分类切换自动请求，用户通过下拉刷新；继续讨论请求失败时的回退状态，本轮不实施代码。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；不修改手机端、后端接口、版本号或现有播放行为。
- 验证：待执行 `git diff --check`、中文替换字符扫描、提交范围检查与文档提交；文档变更无需构建或测试。

## 2026-07-25 11:58 +0800
- 进度：完成短视频合集目录搜索范围术语沉淀；刷新、加载/错误反馈和初期接口范围仍待逐项确认。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；未跟踪 `.superpowers/` 保持原样且不纳入提交。
- 验证：`git diff --check -- CONTEXT.md plan.md` 通过；两份文档的 Unicode 替换字符扫描无匹配；差异检查确认无代码、接口或版本号改动，文档变更无需构建或测试。

## 2026-07-25 11:57 +0800
- 进度：用户确认首期短视频合集目录不提供名称搜索，按后台排序直接浏览；搜索作为后续独立扩展，不增加首期输入框或交互状态。继续讨论刷新与失败反馈，本轮不实施代码。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；不修改手机端、后端接口、版本号或现有播放行为。
- 验证：待执行 `git diff --check`、中文替换字符扫描、提交范围检查与文档提交；文档变更无需构建或测试。

## 2026-07-25 11:54 +0800
- 进度：完成短视频合集内容排序术语沉淀；目录刷新、加载/错误反馈和初期接口范围仍待逐项确认。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；未跟踪 `.superpowers/` 保持原样且不纳入提交。
- 验证：`git diff --check -- CONTEXT.md plan.md` 通过；两份文档的 Unicode 替换字符扫描无匹配；差异检查确认无代码、接口或版本号改动，文档变更无需构建或测试。

## 2026-07-25 11:53 +0800
- 进度：用户确认短视频合集内容页按视频创建时间从新到旧排列，封面列表与连续竖滑播放使用同一顺序；该规则与现有按合集发现短视频的查询一致。继续讨论目录刷新和错误态，本轮不实施代码。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；不修改手机端、后端接口、版本号或现有播放行为。
- 验证：待执行 `git diff --check`、中文替换字符扫描、提交范围检查与文档提交；文档变更无需构建或测试。

## 2026-07-25 11:16 +0800
- 进度：完成短视频合集内容页术语沉淀；合集内内容排序、加载失败和目录刷新策略仍待逐项确认。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；未跟踪 `.superpowers/` 保持原样且不纳入提交。
- 验证：`git diff --check -- CONTEXT.md plan.md` 通过；两份文档的 Unicode 替换字符扫描无匹配；差异检查确认无代码、接口或版本号改动，文档变更无需构建或测试。

## 2026-07-25 11:15 +0800
- 进度：用户确认打开短视频合集后先进入合集内双列视频封面列表，点选某条后才从该条开始连续竖滑播放，不自动播放第一条；可复用短视频发现的浏览、分页和播放语义。继续讨论合集内视频的默认排序，本轮不实施代码。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；不修改手机端、后端接口、版本号或现有播放行为。
- 验证：待执行 `git diff --check`、中文替换字符扫描、提交范围检查与文档提交；文档变更无需构建或测试。

## 2026-07-25 11:08 +0800
- 进度：完成短视频合集目录卡片术语沉淀；合集内播放承接、内容排序和错误恢复仍待逐项确认。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；未跟踪 `.superpowers/` 保持原样且不纳入提交。
- 验证：`git diff --check -- CONTEXT.md plan.md` 通过；两份文档的 Unicode 替换字符扫描无匹配；差异检查确认无代码、接口或版本号改动，文档变更无需构建或测试。

## 2026-07-25 11:07 +0800
- 进度：用户确认短视频合集目录卡片固定显示名称和可播放短视频数量，后台简介非空时显示一行省略；继续讨论点击合集后的内容承接，本轮不实施代码。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；不修改手机端、后端接口、版本号或现有播放行为。
- 验证：待执行 `git diff --check`、中文替换字符扫描、提交范围检查与文档提交；文档变更无需构建或测试。

## 2026-07-25 10:54 +0800
- 进度：完成短视频合集封面回退术语沉淀；目录卡片信息和合集内播放起点仍待逐项确认。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；未跟踪 `.superpowers/` 保持原样且不纳入提交。
- 验证：`git diff --check -- CONTEXT.md plan.md` 通过；两份文档的 Unicode 替换字符扫描无匹配；差异检查确认无代码、接口或版本号改动，文档变更无需构建或测试。

## 2026-07-25 10:53 +0800
- 进度：用户确认短视频合集封面按“管理员设置封面 → 合集内最新可播放短视频缩略图 → 统一占位图”回退；继续讨论目录卡片的信息边界，本轮不实施代码。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；不修改手机端、后端接口、版本号或现有播放行为。
- 验证：待执行 `git diff --check`、中文替换字符扫描、提交范围检查与文档提交；文档变更无需构建或测试。

## 2026-07-24 15:17 +0800
- 进度：完成短视频合集目录排序术语沉淀；目录卡片信息、封面回退和合集内播放起点仍待逐项确认。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；未跟踪 `.superpowers/` 保持原样且不纳入提交。
- 验证：`git diff --check -- CONTEXT.md plan.md` 通过；两份文档的 Unicode 替换字符扫描无匹配；差异检查确认无代码、接口或版本号改动，文档变更无需构建或测试。

## 2026-07-24 15:16 +0800
- 进度：用户确认短视频合集目录按后台 `sort_order` 降序、最近更新时间降序排列；管理员可置顶重要合集，最近维护的同级合集优先展示。继续讨论目录卡片的信息边界，本轮不实施代码。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；不修改手机端、后端接口、版本号或现有播放行为。
- 验证：待执行 `git diff --check`、中文替换字符扫描、提交范围检查与文档提交；文档变更无需构建或测试。

## 2026-07-24 11:38 +0800
- 进度：完成可见短视频合集术语沉淀；目录排序、封面回退和点击后的播放承接仍待逐项确认。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；未跟踪 `.superpowers/` 保持原样且不纳入提交。
- 验证：`git diff --check -- CONTEXT.md plan.md` 通过；两份文档的 Unicode 替换字符扫描无匹配；差异检查确认无代码、接口或版本号改动，文档变更无需构建或测试。

## 2026-07-24 11:37 +0800
- 进度：用户确认手机端短视频合集目录只显示启用且至少包含一条可播放短视频的合集；停用或空合集不展示。继续讨论目录默认排序，本轮不实施代码。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；不修改手机端、后端接口、版本号或现有播放行为。
- 验证：待执行 `git diff --check`、中文替换字符扫描、提交范围检查与文档提交；文档变更无需构建或测试。

## 2026-07-24 11:36 +0800
- 进度：完成短视频合集独立入口术语沉淀；后续仅在用户确认目录规则后再形成实施范围。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；未跟踪 `.superpowers/` 保持原样且不纳入提交。
- 验证：`git diff --check -- CONTEXT.md plan.md` 通过；两份文档的 Unicode 替换字符扫描无匹配；差异检查确认无代码、接口或版本号改动，文档变更无需构建或测试。

## 2026-07-24 11:35 +0800
- 进度：用户确认短视频合集采用首页上方内容分类栏的独立“合集”Tab；它与短视频、电影、电视剧、AV 并列，不新增底部导航项，也不取代图片合集入口。继续讨论目录排序和展示边界，本轮不实施代码。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；不修改手机端、后端接口、版本号或现有播放行为。
- 验证：待执行 `git diff --check`、中文替换字符扫描、提交范围检查与文档提交；文档变更无需构建或测试。

## 2026-07-24 09:10 +0800
- 进度：完成“短视频合集”术语沉淀；仅固化本轮已经确认的内容类型边界，入口位置、目录展示和排序等产品决策继续逐项讨论。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；未跟踪 `.superpowers/` 保持原样且不纳入提交。
- 验证：`git diff --check -- CONTEXT.md plan.md` 通过；两份文档的 Unicode 替换字符扫描无匹配；差异检查确认无代码、接口或版本号改动，文档变更无需构建或测试。

## 2026-07-24 09:09 +0800
- 进度：完成手机端“视频合集页”可行性首轮调研，并与用户收口为仅包含短视频的“短视频合集”；现有合集内分页浏览与连续播放链路可复用，当前缺口是面向用户的合集目录接口、目录页面和入口位置。讨论仍在进行，本轮不实施代码。
- 影响文件：仅 `CONTEXT.md`、`plan.md`；不修改手机端、后端接口、版本号或现有播放行为。
- 验证：待执行 `git diff --check`、中文替换字符扫描与提交范围检查；文档变更无需构建或测试。

## 2026-07-22 17:40 +0800
- 进度：完成禁用 Superpowers skills 的根级规则变更与范围复核；规则同时覆盖自动触发、历史计划、`SKILL.md`、脚本、命名空间和 `.superpowers/` 产物，原复杂任务强制流程已替换为仓库内直接规划与验证流程。
- 影响文件：仅 `AGENTS.md`、`plan.md`；提交时保留并排除既有未跟踪 `.superpowers/`。
- 验证：`git diff --check` 通过；`AGENTS.md`、`plan.md` 替换字符扫描无匹配；规则冲突搜索确认 `AGENTS.md` 中 Superpowers 只出现在两条禁用说明内，原 `grill-with-docs` 强制引用不存在；文档变更无需构建或测试。

## 2026-07-22 17:39 +0800
- 进度：完成根级代理规则调整，禁用 Superpowers 技能包、相关 `SKILL.md`/脚本、`superpowers:` 命名空间与 `.superpowers/` 产物，并移除复杂任务对 `grill-with-docs` 和 Superpowers 子代理流程的强制依赖；复杂任务改为直接依据代码、现有文档和 `plan.md` 完成规划、实施、验证与按需独立评审。
- 影响文件：`AGENTS.md`、`plan.md`；不修改 `.codex/skills/*`、生产代码或 `CONTEXT.md`，既有未跟踪 `.superpowers/` 不纳入提交。
- 验证：待执行 `git diff --check`、替换字符扫描、规则冲突搜索和提交范围检查；文档规则修改无需构建或测试。
...
- 验证：计划内所有生产修改均有明确红灯断言、绿灯命令和独立提交边界；`git diff --check` 通过，计划文件替换字符扫描无输出；待提交计划后选择执行模式。

## 2026-07-21 14:22 +0800
- 进度：根据用户确认的“共享容器优先、页面局部收敛”方案完成布局健壮性设计规格；规格已自检范围、歧义和占位内容，当前仅进入用户文档审阅，不开始生产代码实现。
- 影响文件：`docs/superpowers/specs/2026-07-21-admin-web-layout-hardening-design.md`、`CONTEXT.md`、`plan.md`。
- 验证：规格中的每个修复点均映射到真实数据或独立源码审查证据；`git diff --check` 通过，目标文件替换字符扫描无输出；待提交文档后由用户审阅。

## 2026-07-21 14:16 +0800
- 进度：完成管理端全路由的静态与真实登录数据首轮审查；双人独立复核确认的修复候选集中于共享批量操作条的 375px 收纳、命令面板输入焦点/滚动链、1024px 侧栏下待删除工作台、全宽 Drawer 的固定宽度数值字段、视频详情分集字段、资源表长别名与图片合集 Drawer 的无空格动态文本。四视口几何扫描未发现常规页面级横向滚动；表格横滚仅在自身容器内保留。
- 影响文件：`plan.md`；审查范围为 `admin-web` 全部路由、共享壳层、基础组件和动态真实数据，不修改生产源码。
- 验证：真实管理员会话下逐路由完成 1440px、375px、768px、1024px 几何扫描；`/dashboard`、`/videos`、`/tasks`、`/tv-app`、`/iptv`、`/actors`、`/toolbox/archive-import`、`/toolbox/image-workbench` 完成截图复核。待用户确认修复方案后补充红灯测试。

## 2026-07-21 13:49 +0800
- 进度：确认管理端采用浅色 Precision Ops 单主题；`[data-theme="dark"]` 没有切换入口且现有规格明确排除深色优先，故不纳入本轮页面样式治理。基线自动验证已通过，等待已登录真实浏览器验证的授权条件。
- 影响文件：`plan.md`；已检查 `admin-web/src/{main.js,App.vue,assets/theme.css}` 与既有设计规格。
- 验证：`cd admin-web && npm test -- --reporter=dot` 通过（40 个文件、659 项）；`cd admin-web && npm run build` 通过（仅既有 bundle 体积 warning）。

## 2026-07-20 10:12 +0800
- 进度：管理端布局审查视口与状态矩阵已收口。全部路由统一覆盖 375 / 768 / 1024 / 1440px；除真实数据基线外，还检查加载、空数据、请求失败、极端长文本、Dialog / Drawer / Popover / 下拉展开、表格分页、筛选栏与动作组换行。危险动作只验证确认与取消布局，不执行直接写操作。
- 影响文件：`plan.md`；复用 `CONTEXT.md` 既有“阶段二四视口交互门禁”和长文本边界，不重复新增同义术语。
- 验证：待确认交付形态后，建立逐路由审查矩阵并执行静态检查、浏览器检查与修复验证。

## 2026-07-20 10:10 +0800
- 进度：启动管理端全页面布局审查的 `grill-with-docs` 收口；已确认只治理客观布局缺陷与同类样式不一致，保留现有信息架构和高密度运维风格，不做整站视觉重设计。
- 影响文件：`CONTEXT.md`、`plan.md`；后续审查范围预计覆盖 `admin-web` 全部路由、共享壳层、基础组件与全局样式。
- 验证：待确认视口范围、动态数据覆盖、缺陷分级和交付形式后执行静态审计与浏览器逐页验证。
...
- 验证：新增测试首次因 generation helper 缺失按预期 RED；定向 17 项通过；新鲜 TV 全量单测与 `assembleDebug` 通过（`BUILD SUCCESSFUL`）。待差异/编码检查、提交和独立复审第二轮。
...
- 验证：worktree 基线 `cd admin-web && npm test` 通过（40 files，653 tests）；待提交计划修正后派发 Task 1。
