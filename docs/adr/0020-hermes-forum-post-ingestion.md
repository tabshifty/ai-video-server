...
3. 每轮最多处理固定数量的候选。正文抓取或接口调用失败时，条目增加尝试次数并移动到队尾，让后续候选仍能获得处理机会；普通瞬时抓取失败保留为本地待处理状态，不提交不可变 `failed`。
4. 正文提取成功或明确判定为权限/验证受限后，先把完整检查结果作为 `prepared` 写入候选，再提交 `inspection`。若提交响应不确定，下轮重新 `discover`：`pending` 继续提交，最终态 `duplicate` 使用同一 `prepared` 指纹幂等重放。
5. `inspection` 成功后，候选与对应通知在一次本地原子写入中完成迁移。无需通知的候选直接完成；需要通知的候选留在通知队列，不能依赖 cron stdout 或历史输出文件推断送达。
