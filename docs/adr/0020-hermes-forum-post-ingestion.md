...
附件解析的权限边界：Discuz 附件行中的 `阅读权限: N` 只说明该附件下载项的等级要求，不是帖子页面访问门禁。只要正文可读且链接包含 `mod=attachment`（或其它可识别附件 URL），Hermes 必须继续提取并提交该资源；仅“需要阅读权限”“阅读权限高于”“本帖隐藏的内容需要”或明确要求登录/验证的完整门禁文案才可将页面判为 `restricted`。
